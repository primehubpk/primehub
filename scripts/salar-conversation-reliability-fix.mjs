...
const route = 'app/api/salar/chat/route.ts';
const store = 'lib/salar/chatStore.ts';
...
`, 'runProviderTarget json mode forwarding');
...
`, 'persist product selections in AI history');
...
replaceOnce(widget,
`<p className="text-[8px] text-black/40">Advance Rs. 300 · balance after ready-order video</p>`,
`<p className="text-[8px] text-black/40">Live bill from selected products</p>`, 'remove order card advance hardcoding');

console.log('Salar conversation reliability patch applied.');
