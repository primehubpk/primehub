...
text = text.replace("updateVariant(variant.id, { color: event.target.value })", "updateVariant(index, { color: event.target.value })")
text = text.replace("updateVariant(variant.id, { size: event.target.value })", "updateVariant(index, { size: event.target.value })")
text = text.replace("updateVariant(variant.id, { stock: event.target.value })", "updateVariant(index, { stock: event.target.value })")
text = text.replace("setEditingVariantId(null)", "setEditingVariantIndex(null)")
text = text.replace("removeVariant(variant.id)", "removeVariant(index)")
text = text.replace("setEditingVariantId(variant.id)", "setEditingVariantIndex(index)")

if 'editingVariantId' in text or 'setEditingVariantId' in text:
    raise SystemExit('Old variant id editing state still present after patch')

path.write_text(text)

for temp in [Path('.github/workflows/temp-bulk-editor-patch.yml'), Path('scripts/temp_bulk_editor_patch.py')]:
    if temp.exists():
        temp.unlink()
