        ...
        <div className="rounded-[28px] border border-black/5 bg-white p-4 shadow-[0_14px_38px_rgba(20,20,15,0.07)] sm:p-5">
          <div className="grid gap-2 sm:grid-cols-2">
            {benefits.map(({ icon: Icon, title, text, tone }) => (
              <div key={title} className="rounded-[20px] border border-black/5 bg-[#FAFAF7] p-4">
                <div className="flex items-start gap-3"><div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${tone}`}><Icon size={20} /></div><div className="min-w-0"><h3 className="text-sm font-black">{title}</h3><p className="mt-1 text-[10px] leading-5 text-black/50">{text}</p></div></div>
              </div>
            ))}
          </div>
