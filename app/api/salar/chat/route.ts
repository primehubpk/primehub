...
function publicChat(chat: SalarCustomerChat | null) {
  ...
  return {
    ...
    messages: chat.messages.map(({ imageAnalysis: _imageAnalysis, ...message }) => message),
  };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const aiHistory = salarAiHistory(chat, 30);
    ...
    const recentReferences = recentCustomerProductReferences(chat, 30);
    ...
    if (result.imageUnderstanding) {
      ...
      if (customerIndex != null) {
        chat = {
          ...chat,
          messages: chat.messages.map((stored, index) => index === customerIndex
            ? { ...stored, imageAnalysis: cleanText(result.imageUnderstanding, 1600) }
            : stored),
        };
      }
    }
    ...
    const { imageUnderstanding: _privateImageUnderstanding, ...publicResult } = result;
    return NextResponse.json({
      success: true,
      ...publicResult,
      salarPaused: false,
