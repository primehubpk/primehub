...
export async function POST(request:Request){
  ...
  try{
    ...
    try{const brain=await buildSalarBrainPrompt();const isCollectionFollowUp=collectionFollowUp(stored);const systemPrompt=`${brain}\n\n# Live tool rules\nNever invent catalogue, stock, price, delivery, payment or website facts. There is ONE Worker: catalogue, knowledge, vision, order, moderate. Moderation is server-enforced before you. Never teach illegal or harmful instructions. Order closing is server-enforced: Rs 300 screenshot first, then one field at a time. Never claim advance is verified. Website order is auto-saved once summary is complete. For broad shopping call catalogue with q only, list every returned collection and ask which one. For website facts use knowledge. Keep replies short, meetha, adab se. Shopping starts should identify you as Salar from PrimeHub Mall. Phone ${PHONE} if verified tools fail.`;const llmMessages:SalarLlmMessage[]=[{role:'system',content:systemPrompt},...historyForModel(stored)];let rounds=0;
      while(true){const completion=await chatCompletionWithTools({messages:llmMessages,tools:rounds<4?TOOLS:[]});if(!completion.toolCalls.length){assistantText=completion.text.trim();break;}rounds++;usedTool=true;llmMessages.push({role:'assistant',content:completion.text||'',tool_calls:completion.toolCalls.map(c=>({id:c.id,type:'function',function:{name:c.name,arguments:c.rawArguments||JSON.stringify(c.arguments)}}))});for(const call of completion.toolCalls){const args=safeArgs(call.arguments);const result=call.name==='catalogue'?await runCustomerWorker(request,{job:'catalogue',payload:args,conversationId:conversation.id}):call.name==='knowledge'?await runCustomerWorker(request,{job:'knowledge',payload:args,conversationId:conversation.id}):{found:false,reason:`Unsupported tool ${call.name}`};if((result as any)?.found===false)failedTool=true;else successfulTool=true;if((result as any)?.type==='collections')lastCollections=Array.isArray((result as any).collections)?(result as any).collections:[];if((result as any)?.type==='products'&&Array.isArray((result as any).products)){products=(result as any).products;productCollectionName=String((result as any)?.collection?.name||'').trim();lastCollections=null;}llmMessages.push({role:'tool',name:call.name,tool_call_id:call.id,content:JSON.stringify(result)});}}
      if(!usedTool&&(shoppingLike(text)||isCollectionFollowUp)){const fallback:any=await runCustomerWorker(request,{job:'catalogue',payload:isCollectionFollowUp?{collection:text}:{q:text},conversationId:conversation.id});usedTool=true;if(fallback?.found===false)failedTool=true;else successfulTool=true;if(fallback?.type==='collections')lastCollections=Array.isArray(fallback.collections)?fallback.collections:[];if(fallback?.type==='products'&&Array.isArray(fallback.products)){products=fallback.products;productCollectionName=String(fallback?.collection?.name||'').trim();lastCollections=null;}}
      if(failedTool&&!successfulTool)assistantText=noVerifiedResult();else if(products.length)assistantText=`Yeh ${productCollectionName||'is collection'} ke verified PrimeHub products hain. Aap kisi card ka number ya naam mention karke size, material ya order pooch sakte hain.`;else if(lastCollections)assistantText=collectionReply(lastCollections);else if(!assistantText)assistantText=noVerifiedResult();
    }catch(error){console.error('[salar-messages] completion failed',{conversationId:conversation.id,error:error instanceof Error?error.message:'unknown'});assistantText=SALAR_SAFE_ERROR_MESSAGE;products=[];}
