...
type UserWallet = { points?: number; streak?: number; lastCheckIn?: string; lastSpin?: string; coupons?: string[]; vouchers?: VoucherReward[]; freeProducts?: ProductReward[]; freeDeliveryCredits?: number; history?: RewardHistoryEntry[] };
...
function choosePrize(prizes: Prize[]) { const active = prizes.filter(p => p.active !== false && Number(p.probability) > 0 && Number(p.stock ?? 1) > 0).slice(0, 5); const total = active.reduce((sum, p) => sum + Number(p.probability || 0), 0); if (!active.length || total <= 0) return null; let cursor = Math.random() * total; for (const prize of active) { cursor -= Number(prize.probability || 0); if (cursor <= 0) return prize; } return active[active.length - 1]; }
async function rewardSettings() { if (isSupabaseWriteConfigured()) { try { const r = await sb('settings?id=eq.rewards&select=payload&limit=1'); const rows = await r.json(); const data = rows?.[0]?.payload || {}; if (Array.isArray(data.spinWheelSlots) && data.spinWheelSlots.length) return data; } catch (error) { console.error('Supabase guest reward settings read failed; using Firebase fallback:', error); } } const snap = await getAdminDb().collection('settings').doc('rewards').get(); return snap.data() || {}; }
async function readGuest(id: string): Promise<GuestWallet> { if (isSupabaseWriteConfigured()) { try { const r = await sb(`user_rewards?id=eq.${encodeURIComponent(`guest:${id}`)}&select=payload&limit=1`); const rows = await r.json(); return rows?.[0]?.payload || {}; } catch (error) { console.error('Supabase guest wallet read failed; using Firebase fallback:', error); } } const snap = await getAdminDb().collection('user_rewards').doc(`guest_${id}`).get(); return (snap.data() || {}) as GuestWallet; }
async function writeGuest(id: string, wallet: GuestWallet) { if (isSupabaseWriteConfigured()) { await sb('user_rewards?on_conflict=id', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({ id: `guest:${id}`, user_id: null, payload: wallet, authoritative_source: 'supabase', mirror_status: 'pending', updated_at: new Date().toISOString() }) }); try { await getAdminDb().collection('user_rewards').doc(`guest_${id}`).set({ ...wallet, updatedAt: FieldValue.serverTimestamp() }, { merge: true }); } catch {} return; } await getAdminDb().collection('user_rewards').doc(`guest_${id}`).set({ ...wallet, updatedAt: FieldValue.serverTimestamp() }, { merge: true }); }
async function readUser(uid: string): Promise<UserWallet> { if (isSupabaseWriteConfigured()) { try { const r = await sb(`user_rewards?id=eq.${encodeURIComponent(uid)}&select=payload&limit=1`); const rows = await r.json(); return rows?.[0]?.payload || {}; } catch (error) { console.error('Supabase user wallet read failed; using Firebase fallback:', error); } } const snap = await getAdminDb().collection('user_rewards').doc(uid).get(); return (snap.data() || {}) as UserWallet; }
async function writeUser(uid: string, wallet: UserWallet) { if (isSupabaseWriteConfigured()) { await sb('user_rewards?on_conflict=id', { method: 'POST', headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }, body: JSON.stringify({ id: uid, user_id: uid, payload: wallet, authoritative_source: 'supabase', mirror_status: 'pending', updated_at: new Date().toISOString() }) }); } try { await getAdminDb().collection('user_rewards').doc(uid).set({ ...wallet, updatedAt: FieldValue.serverTimestamp() }, { merge: true }); } catch {} }
...
export async function POST(request: Request) {
  try {
    ...
    if (body?.action === 'claim') {
      ...
      const voucherReward: VoucherReward | null = prize.type === 'coupon' ? { id: guest.pendingPrizeToken, code: voucher, amount: Math.max(0, Number(prize.voucherAmount || 0)), name: prize.name, createdAt: now } : null;
      const productReward: ProductReward | null = prize.type === 'product' ? { id: guest.pendingPrizeToken, productId: String(prize.productId || ''), name: prize.name, imageUrl: prize.imageUrl, status: 'pending', createdAt: now } : null;
      const next = { ...current, points: Number(current.points || 0) + points, lastSpin: guest.lastSpin || current.lastSpin, coupons: voucher ? [...(current.coupons || []), voucher] : (current.coupons || []), vouchers: voucherReward ? [...(current.vouchers || []), voucherReward] : (current.vouchers || []), freeProducts: productReward ? [...(current.freeProducts || []), productReward] : (current.freeProducts || []), freeDeliveryCredits: Number(current.freeDeliveryCredits || 0) + (prize.type === 'free-delivery' ? 1 : 0), history };
