...
export default function DealsPage() {
  ...
  const weeklyDeals = useMemo(
    () => (settings.weeklyDeals || []).filter((deal) => deal.productId),
  ...
  const addToCart = (deal: WeeklyDeal, status: DealStatus) => {
    ...
    const price =
      status === 'live' && deal.active !== false && dealPrice > 0
        ? dealPrice
        : regularPrice;
    if (price <= 0) return;
    ...
    addItem({
      ...
      originalPrice:
        regularPrice > price
          ? regularPrice
          : Number(product.originalPrice || regularPrice),
    ...
    setAddingId(deal.id);
    window.setTimeout(
      () => setAddingId((current) => (current === deal.id ? null : current)),
      1200,
    );
  };
      ...
      <div className="mx-auto max-w-6xl px-4 py-5 md:px-6 md:py-7">
        ...
        <header className="mt-5 overflow-hidden rounded-[30px] bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-black/50">
            Every deal repeats automatically each week using Pakistan time. Past
            days never expire — they roll forward to their next occurrence.
          </p>
        </header>
        ...
        {!weeklyDeals.length ? (
          <section className="mt-5 rounded-[30px] bg-white p-10 text-center shadow-sm">
            ...
            <p className="mt-2 text-xs text-black/45">
              Add real products and special prices from Admin → Deals.
            </p>
          </section>
        ) : (
          <div className="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {DAYS.map(({ key, label }) => {
              ...
              if (!deal) {
                return (
                  <article
                    key={key}
                    className="rounded-[28px] border border-dashed border-black/10 bg-white p-5 text-center shadow-sm"
                  >
                    ...
                    <p className="mt-1 text-xs text-black/40">
                      Admin has not assigned a product yet.
                    </p>
                  </article>
                );
              }
              ...
              return (
                <article
                  key={deal.id}
                  className="group overflow-hidden rounded-[28px] border border-black/5 bg-white shadow-[0_12px_40px_rgba(0,0,0,0.06)] transition hover:-translate-y-0.5"
                >
                  <div className="relative aspect-square bg-[#F4F4F1]">
                    ...
                    <div className="absolute left-3 right-3 top-3 flex items-start justify-between gap-2">
                      <span
                        className={`rounded-full border px-2.5 py-1.5 text-[8px] font-black uppercase tracking-wide ${statusStyles(status)}`}
                      >
                        {statusLabel(status)}
                      </span>
                      {discount > 0 && (
                  ...
                  <div className="p-4">
                    ...
                    <h2 className="mt-1.5 line-clamp-2 min-h-[44px] text-lg font-black">
                      {title}
                    </h2>
                    ...
                    <div className="mt-3 flex items-end gap-2">
                      ...
                      {regularPrice > dealPrice && (
                        <span className="pb-0.5 text-xs text-black/35 line-through">
                    ...
                    <p className="mt-1 text-[10px] text-black/40">
                      {status === 'live'
                        ? 'Special price active today.'
                        : status === 'upcoming'
                          ? 'Special price unlocks on this day.'
                          : 'Special price returns next week.'}
                    </p>
                    ...
                    <div className="mt-4 flex gap-2">
                      ...
                      <Link
                        href={`/deals/${key}`}
                        className="inline-flex items-center justify-center rounded-xl border border-black/10 px-3 py-3 text-[10px] font-black"
                      >
                        VIEW
                      </Link>
                    </div>
