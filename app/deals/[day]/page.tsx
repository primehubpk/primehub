          ...
          <section className={`relative mt-4 overflow-hidden rounded-[32px] border shadow-sm ${live ? 'border-emerald-300/20 bg-white shadow-[0_30px_100px_rgba(16,185,129,.14)]' : 'border-black/6 bg-white'}`}>
            ...
            <div className="relative grid lg:grid-cols-[1.05fr_.95fr]">
              ...
              <div className="flex flex-col p-5 sm:p-7 md:p-9"><div className="flex items-center gap-2 text-[#E1352B]"><Sparkles size={15} /><p className="text-[9px] font-black uppercase tracking-[0.22em]">{live ? 'LIMITED TIME OFFER' : status === 'upcoming' ? 'NEXT WEEKLY EVENT' : 'REGULAR STOREFRONT PRICE'}</p></div><p className="mt-2 text-[10px] font-black uppercase tracking-[0.18em] text-black/35">{DAYS[day].toUpperCase()} DEAL</p><h2 className="mt-2 text-3xl font-black leading-tight tracking-tight text-[#14140F] md:text-4xl">{title}</h2>
                ...
                <div className="mt-6 grid gap-2 sm:grid-cols-2">{live ? <button type="button" onClick={buyNow} disabled={!available} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-[#14140F] px-5 text-sm font-black text-white shadow-lg transition hover:-translate-y-0.5 hover:shadow-xl disabled:cursor-not-allowed disabled:opacity-40"><ShoppingBag size={18} /> BUY NOW — Rs. {displayedPrice.toLocaleString()}</button> : <Link href={`/deals/${today}`} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-[#14140F] px-5 text-sm font-black text-white"><Zap size={17} fill="currentColor" /> VIEW TODAY'S LIVE DEAL</Link>}<Link href={`/product/${product?.id}`} className="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl border border-black/10 px-5 text-xs font-black text-[#14140F] hover:bg-black/[0.02]">View Full Details <ChevronRight size={15} /></Link></div>
