...
export default function DayDealPage() {
  ...
  const addCurrentProduct = () => {
    ...
    addItem({
      ...
      name: title,
      price,
      originalPrice: compareAtPrice > price ? compareAtPrice : regularPrice,
  ...
  const addWeeklyProduct = (itemDay: Weekday) => {
    ...
    const itemStock = Number(itemProduct.stock ?? itemProduct.quantity ?? 0);
    ...
    addItem({
      id: itemProduct.id,
      name: itemProduct.title || `${DAYS[itemDay]} Deal`,
      price,
      originalPrice: Number(itemProduct.originalPrice || 0) > price ? Number(itemProduct.originalPrice) : regular,
      image: itemProduct.imageUrl || undefined,
      imageUrl: itemProduct.imageUrl || undefined,
      ...(itemStatus === 'live' ? { dealDay: itemDay } : {}),
    });
