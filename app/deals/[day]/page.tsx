...
export default function DayDealPage() {
  ...
  useEffect(() => {
    setCountdown(secondsUntilPakistanMidnight());
    const timer = window.setInterval(() => setCountdown(secondsUntilPakistanMidnight()), 1000);
  ...
  return <main className="min-h-screen bg-neutral-50 pb-28">
    <div className="mx-auto max-w-6xl px-4 py-5 md:px-6 md:py-7">
      ...
      <header className="mt-5 rounded-[28px] bg-white p-5 shadow-sm md:p-7">
        <p className="text-[9px] font-black uppercase tracking-[0.25em] text-[#E1352B]">{label} Deal</p>
        <h1 className="mt-1 text-3xl font-black tracking-tight md:text-5xl">{title}</h1>
        <p className="mt-2 max-w-2xl text-sm leading-6 text-black/50">{status === 'live' ? 'Today’s special offer is live in Pakistan time.' : status === 'upcoming' ? `Preview the ${label} offer. The discounted price unlocks when ${label} arrives.` : `Preview the ${label} offer. The discounted price returns next ${label}.`}</p>
      </header>
        ...
        <div className="grid lg:grid-cols-[1.05fr_.95fr]">
          <div className="relative aspect-square bg-[#F4F4F1] lg:aspect-auto lg:min-h-[560px]">
            ...
            <div className="absolute left-4 top-4 flex flex-wrap gap-2">
              <span className={`rounded-full border px-3 py-1.5 text-[9px] font-black ${statusStyles(status)}`}>{statusLabel(status)}</span>
              {discount > 0 && <span className="rounded-full bg-[#E1352B] px-3 py-1.5 text-[9px] font-black text-white">-{discount}% OFF</span>}
            </div>
          </div>
          ...
          <div className="p-6 md:p-10">
            ...
            <div className="mt-3 flex flex-wrap items-end gap-3">
              {regularPrice > dealPrice && <span className="text-lg text-black/35 line-through">Rs. {regularPrice.toLocaleString()}</span>}
              <span className="font-[family-name:var(--font-mono)] text-4xl font-black text-[#E1352B]">Rs. {dealPrice > 0 ? dealPrice.toLocaleString() : regularPrice.toLocaleString()}</span>
            </div>
            {live && savings > 0 && <div className="mt-3 inline-flex rounded-full bg-emerald-50 px-3 py-1.5 text-[10px] font-black text-emerald-700">YOU SAVE Rs. {savings.toLocaleString()} ({discount}% OFF)</div>}

            {live && <div className="mt-5 rounded-2xl border border-black/8 bg-neutral-50 px-4 py-3"><p className="text-[9px] font-black uppercase tracking-[0.22em] text-black/40">Deal ends in</p><div className="mt-1 font-[family-name:var(--font-mono)] text-2xl font-black tracking-wide text-[#14140F]">{formatCountdown(countdown)}</div></div>}

            <button type="button" disabled={!product || !available || !live || dealPrice <= 0} onClick={() => deal && addToCart(deal, 'live')} className="mt-5 flex w-full items-center justify-center gap-2 rounded-2xl bg-[#14140F] px-5 py-4 text-sm font-black text-white disabled:cursor-not-allowed disabled:opacity-35"><ShoppingBag size={18} /> BUY NOW - Rs. {dealPrice > 0 ? dealPrice.toLocaleString() : regularPrice.toLocaleString()}</button>
            {!live && product && available && <p className="mt-2 text-center text-[10px] font-bold text-black/40">This product is available now at its regular storefront price: Rs. {regularPrice.toLocaleString()}.</p>}
      ...
      <section className="mt-8 rounded-[30px] bg-neutral-50 p-1">
        <div className="rounded-[28px] bg-white p-5 shadow-sm md:p-7">
          <div className="flex items-end justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[0.25em] text-[#E1352B]">Keep exploring</p><h2 className="mt-1 text-2xl font-black">EXPLORE WEEKLY DEALS</h2></div><Link href="/deals" className="text-[9px] font-black uppercase tracking-[0.18em] text-black/40">View all</Link></div>
          {!weeklyDeals.length ? <div className="mt-5 rounded-2xl border border-dashed border-black/10 bg-neutral-50 p-6 text-center"><p className="text-xs font-bold text-black/40">No weekly deals scheduled yet.</p></div> : <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
            {DAYS.map(({ key, label: itemLabel }) => {
              const itemDeal = weeklyDeals.find((item) => item.day === key); if (!itemDeal) return null;
              ...
              const itemDiscount = itemRegular > itemDealPrice && itemDealPrice > 0 ? Math.round(((itemRegular - itemDealPrice) / itemRegular) * 100) : 0;
              const itemImage = imageOf(itemProduct, itemDeal);
              const itemTitle = itemProduct?.title || itemDeal.title || `${itemLabel} Deal`;
              const itemAvailable = Boolean(itemProduct && Number(itemProduct.stock ?? 0) > 0 && itemRegular > 0);
              const itemButtonPrice = itemStatus === 'live' && itemDealPrice > 0 ? itemDealPrice : itemRegular;
              return <article key={itemDeal.id} className="overflow-hidden rounded-2xl border border-black/6 bg-white shadow-sm">
                <div className="relative h-44 bg-[#F4F4F1] sm:h-48">{itemImage ? <img src={itemImage} alt={itemTitle} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-[10px] font-bold text-black/25">No image</div>}
                  <div className="absolute left-2 top-2 flex flex-wrap gap-1.5"><span className={`rounded-full border px-2 py-1 text-[7px] font-black ${statusStyles(itemStatus)}`}>{statusLabel(itemStatus)}</span>{itemDiscount > 0 && <span className="rounded-full bg-[#E1352B] px-2 py-1 text-[7px] font-black text-white">-{itemDiscount}%</span>}</div>
                </div>
                <div className="p-3"><p className="text-[8px] font-black uppercase tracking-[0.16em] text-[#E1352B]">{itemLabel} Deal</p><h3 className="mt-1 line-clamp-2 min-h-[36px] text-sm font-black">{itemTitle}</h3>
                  <div className="mt-2 flex items-center gap-2"><span className="text-base font-black text-[#E1352B]">Rs. {itemButtonPrice.toLocaleString()}</span>{itemStatus === 'live' && itemRegular > itemDealPrice && <span className="text-[10px] text-black/35 line-through">Rs. {itemRegular.toLocaleString()}</span>}</div>
                  <button type="button" disabled={!itemAvailable || addingId === itemDeal.id} onClick={() => addToCart(itemDeal, itemStatus)} className="mt-3 w-full rounded-xl bg-[#14140F] px-2 py-2.5 text-[9px] font-black text-white disabled:cursor-not-allowed disabled:opacity-35">{addingId === itemDeal.id ? 'ADDED' : itemStatus === 'live' ? `BUY NOW AT DEAL PRICE - Rs. ${itemDealPrice.toLocaleString()}` : `ADD TO CART - Rs. ${itemRegular.toLocaleString()}`}</button>
                </div>
              </article>;
            })}
          </div>}
