...
function nextDealUnlockISO(day: Weekday, today: Weekday) {
  ...
  return pakistanMidnightISO(delta === 0 ? 1 : delta + 1);
}
...
export default function DayDealPage() {
  ...
  useEffect(() => {
    ...
    async function loadWeeklyProducts() {
      ...
      const assigned = weeklyDeals.filter((item) => item.productId);
      if (!assigned.length) {
        setWeeklyProducts({});
        return;
      }
      ...
      const entries = await Promise.all(
        assigned.map(async (item) => {
          try {
            ...
            return [item.day, null] as const;
          }
        })
      );
      if (!cancelled) setWeeklyProducts(Object.fromEntries(entries));
      if (!cancelled) setWeeklyProductsLoading(false);
    }
    loadWeeklyProducts();
    return () => {
      cancelled = true;
  ...
  return (
    <main className={`min-h-screen overflow-x-hidden pb-28 ${live ? 'bg-[#0E100F]' : 'bg-[#F4F4F1]'}`}>
      {live && <DealCelebration />}
      <div className="mx-auto max-w-6xl px-4 py-4 md:px-6 md:py-6">
        <div className="flex items-center justify-between">
          <Link href="/" className={`inline-flex items-center gap-2 rounded-full px-4 py-2.5 text-[10px] font-black shadow-sm ${live ? 'bg-white/10 text-white backdrop-blur' : 'bg-white text-[#14140F]'}`}><ArrowLeft size={14} /> Back to Home</Link>
          <span className={`hidden text-[9px] font-black uppercase tracking-[0.25em] sm:block ${live ? 'text-white/35' : 'text-black/35'}`}>PrimeHub Live Deal Experience</span>
        </div>
        ...
        <section className={`relative mt-4 overflow-hidden rounded-[32px] p-6 shadow-[0_25px_80px_rgba(0,0,0,0.16)] md:p-9 ${live ? 'border border-emerald-300/20 bg-[radial-gradient(circle_at_80%_20%,rgba(16,185,129,.18),transparent_30%),radial-gradient(circle_at_20%_80%,rgba(225,53,43,.14),transparent_35%),#111512] text-white' : status === 'upcoming' ? 'bg-[#10202A] text-white' : 'bg-[#20201C] text-white'}`}>
          ...
          <div className="relative">
            <div className="flex flex-wrap items-center gap-2">
              <span className={`rounded-full px-3 py-1.5 text-[9px] font-black uppercase tracking-wider shadow-lg ${live ? 'bg-[#E1352B] text-white shadow-red-500/30' : 'bg-white/10 text-white/70'}`}>{live ? 'LIVE NOW' : status === 'upcoming' ? 'COMING SOON' : 'DEAL ENDED'}</span>
              {live && <span className="rounded-full bg-[#FFB020] px-3 py-1.5 text-[9px] font-black text-[#14140F] shadow-lg">TODAY ONLY</span>}
            </div>
            <p className="mt-5 text-[9px] font-black uppercase tracking-[0.3em] text-[#FFB020]">One day special offer</p>
            <h1 className="mt-2 text-3xl font-black tracking-tight md:text-6xl">{DAYS[day]} Big Deal</h1>
            <p className="mt-2 max-w-2xl text-xs leading-5 text-white/55 md:text-sm">{live ? 'A luxury flash-sale event is live right now. The special price expires at midnight Pakistan time.' : status === 'upcoming' ? `The ${DAYS[day]} event is scheduled next. Its special price stays locked until the event opens.` : `The ${DAYS[day]} event has ended. The product is shown at its regular storefront price.`}</p>
          </div>
        </section>
        ...
        {!scheduled ? (
          ...
          <section className={`relative mt-4 overflow-hidden rounded-[32px] border shadow-sm ${live ? 'border-emerald-300/20 bg-white shadow-[0_30px_100px_rgba(16,185,129,.14)]' : 'border-black/6 bg-white'}`}>
            ...
            <div className="relative grid lg:grid-cols-[1.05fr_.95fr]">
              <div className="relative min-h-[360px] bg-[#EEEDE7] lg:min-h-[610px]">
                ...
                <div className="absolute left-4 top-4 flex flex-wrap gap-2">
                  {live && <span className="flex items-center gap-1.5 rounded-full bg-[#E1352B] px-3 py-1.5 text-[9px] font-black text-white shadow-lg"><Zap size={11} fill="currentColor" /> LIVE NOW</span>}
                  ...
                  {!live && status === 'upcoming' && <span className="rounded-full bg-sky-500/90 px-3 py-1.5 text-[9px] font-black text-white">LOCKED</span>}
                </div>
              ...
              <div className="flex flex-col p-5 sm:p-7 md:p-9">
                ...
                {live && <div className="mt-5 rounded-[24px] border border-red-500/10 bg-[#FFF7F4] p-4 shadow-inner">
                  <p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#E1352B]">🔥 DEAL ENDS IN</p>
                  <div className="mt-2"><DealCountdown target={liveCountdownTarget} onExpired={() => setExpired(true)} /></div>
                </div>}
                ...
                <div className="mt-6">
                  ...
                  {status === 'upcoming' && <p className="text-[10px] font-black uppercase tracking-[0.16em] text-black/40">Regular Price</p>}
                  ...
                  {status === 'upcoming' && configuredDealPrice > 0 && <div className="mt-4 rounded-2xl border border-sky-500/15 bg-sky-50 p-4"><p className="text-[9px] font-black uppercase tracking-[0.15em] text-sky-700">Locked Deal Price</p><p className="mt-1 text-2xl font-black text-sky-900">🔒 Rs. {configuredDealPrice.toLocaleString()}</p><p className="mt-1 text-[9px] font-black uppercase tracking-[0.15em] text-sky-700">UNLOCKS {DAYS[day].toUpperCase()}</p><div className="mt-3"><DealCountdown target={upcomingCountdownTarget} /></div></div>}
                  {status === 'ended' && <p className="mt-2 text-[10px] font-bold text-black/40">This discounted event price is no longer active.</p>}
                </div>
        ...
        <section className={`mt-6 rounded-[32px] p-5 sm:p-7 ${live ? 'border border-white/10 bg-[#171A18] text-white shadow-[0_25px_80px_rgba(16,185,129,.08)]' : 'bg-[#14140F] text-white'}`}>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-7">
            {DAY_ORDER.map((itemDay) => {
              ...
              const itemImage = itemProduct?.imageUrl || '';
              const active = itemDay === day;
              const itemDealPrice = Number(dealItem?.dealPrice || 0);
              const itemRegularPrice = itemProduct ? (Number(itemProduct.originalPrice) > Number(itemProduct.price) ? Number(itemProduct.originalPrice) : Number(itemProduct.price)) : 0;
              const showLivePrice = itemStatus === 'live' && itemDealPrice > 0;
              const showLockedPrice = itemStatus === 'upcoming' && itemDealPrice > 0;
              const imageBroken = Boolean(imageErrors[itemDay]);
              return (
                <Link key={itemDay} href={`/deals/${itemDay}`} className={`group relative overflow-hidden rounded-[22px] border transition duration-300 hover:-translate-y-1 hover:shadow-xl ${active ? 'border-white bg-white text-[#14140F] shadow-lg' : 'border-white/10 bg-white/[0.06] text-white hover:border-white/20'}`}>
                  <div className="relative aspect-[4/3] overflow-hidden bg-white/10">
                    {itemImage && !imageBroken ? <img src={itemImage} alt={`${DAYS[itemDay]} deal product`} onError={() => setImageErrors((current) => ({ ...current, [itemDay]: true }))} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full w-full flex-col items-center justify-center gap-1 bg-gradient-to-br from-white/10 to-white/5 px-3 text-center"><Tag size={18} className="opacity-30" /><span className="text-[8px] font-black uppercase tracking-wider opacity-45">No Deal Scheduled</span></div>}
                    <div className="absolute inset-0 bg-gradient-to-t from-black/65 via-transparent to-black/5" />
                    <span className={`absolute left-2 top-2 rounded-full border px-2 py-1 text-[7px] font-black uppercase tracking-wider backdrop-blur ${statusClass(itemStatus)}`}>{dealItem ? statusLabel(itemStatus) : 'NO DEAL'}</span>
                  </div>
                  <div className="p-3">
                    <div className="flex items-center justify-between gap-1"><span className="text-[10px] font-black">{DAYS[itemDay]}</span><ChevronRight size={12} className="shrink-0 opacity-40" /></div>
                    {showLivePrice && <p className="mt-2 font-[family-name:var(--font-mono)] text-sm font-black text-[#E1352B]">Rs. {itemDealPrice.toLocaleString()}</p>}
                    {showLockedPrice && <><p className="mt-2 text-[8px] font-bold opacity-45">Regular Rs. {itemRegularPrice.toLocaleString()}</p><p className={`mt-0.5 font-[family-name:var(--font-mono)] text-xs font-black ${active ? 'text-sky-700' : 'text-sky-300'}`}>🔒 Rs. {itemDealPrice.toLocaleString()}</p></>}
                    {itemStatus === 'ended' && itemRegularPrice > 0 && <p className="mt-2 font-[family-name:var(--font-mono)] text-xs font-black opacity-55">Rs. {itemRegularPrice.toLocaleString()}</p>}
                    {itemStatus === 'upcoming' && <p className="mt-1 text-[7px] font-black uppercase tracking-wider text-sky-300">Unlocks {DAYS[itemDay]}</p>}
                    {itemStatus === 'live' && <p className="mt-1 text-[7px] font-black uppercase tracking-wider text-emerald-400">Live today</p>}
                    {itemStatus === 'ended' && <p className="mt-1 text-[7px] font-black uppercase tracking-wider opacity-40">Event ended</p>}
                  </div>
                </Link>
              );
            })}
