...
export default function DayDealPage() {
  ...
  const purchasePrice = live ? dealPrice : regularPrice;
  ...
  return <main className="min-h-screen bg-neutral-50 pb-28">
    <div className="mx-auto max-w-6xl px-4 py-5 md:px-6 md:py-7">
      ...
      <header className="mt-5 rounded-[28px] bg-white p-5 shadow-sm md:p-7">
        ...
        <p className="mt-2 max-w-2xl text-sm leading-6 text-black/50">{live ? 'Today’s special offer is live in Pakistan time.' : `This offer is scheduled for ${label}. The special price unlocks on ${label}.`}</p>
      </header>
      ...
      <section className="mt-5 overflow-hidden rounded-[30px] border border-black/5 bg-white shadow-sm">
        <div className="grid lg:grid-cols-[1.05fr_.95fr]">
          <div className="relative aspect-square bg-[#F4F4F1] lg:aspect-auto lg:min-h-[560px]">
            ...
            <div className="absolute left-4 top-4 flex flex-wrap gap-2"><span className={`rounded-full border px-3 py-1.5 text-[9px] font-black ${statusStyles(status)}`}>{statusLabel(status, day)}</span>{discount > 0 && <span className="rounded-full bg-[#E1352B] px-3 py-1.5 text-[9px] font-black text-white">-{discount}% OFF</span>}</div>
          </div>
          ...
          <div className="p-6 md:p-10">
            ...
            {live ? (
              ...
              <>
                <div className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 p-4"><p className="text-[11px] font-black text-sky-800">🔒 DEAL LOCKED — UNLOCKS ON {label.toUpperCase()}</p><p className="mt-1 text-xs text-sky-700/80">Current storefront price is available today; the discounted weekly price stays locked until {label}.</p></div>
                <div className="mt-5 space-y-2 rounded-2xl bg-neutral-50 p-4"><div className="flex items-center justify-between text-sm"><span className="font-bold text-black/55">Current Storefront Price</span><span className="font-black">Rs. {regularPrice.toLocaleString()}</span></div><div className="flex items-center justify-between text-sm"><span className="font-bold text-black/55">Upcoming Deal Price</span><span className="font-black text-[#E1352B]">Rs. {dealPrice.toLocaleString()} <span className="text-[10px] text-black/35">locked</span></span></div></div>
      ...
      <section className="mt-8 rounded-[30px] bg-neutral-50 p-1"><div className="rounded-[28px] bg-white p-5 shadow-sm md:p-7"><div className="flex items-end justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-[0.25em] text-[#E1352B]">Keep exploring</p><h2 className="mt-1 text-2xl font-black">EXPLORE WEEKLY DEALS</h2></div><Link href="/deals" className="text-[9px] font-black uppercase tracking-[0.18em] text-black/40">View all</Link></div>{!weeklyDeals.length ? <div className="mt-5 rounded-2xl border border-dashed border-black/10 bg-neutral-50 p-6 text-center"><p className="text-xs font-bold text-black/40">No weekly deals scheduled yet.</p></div> : <div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">{DAYS.map(({ key, label: itemLabel }) => { const itemDeal = weeklyDeals.find((item) => item.day === key); if (!itemDeal) return null; const itemProduct = products[itemDeal.productId] || null; const itemStatus = statusForDay(key, today); const itemRegular = regularPriceOf(itemProduct, itemDeal); const itemDealPrice = Number(itemDeal.dealPrice || 0); const itemDiscount = itemRegular > itemDealPrice && itemDealPrice > 0 ? Math.round(((itemRegular - itemDealPrice) / itemRegular) * 100) : 0; const itemImage = imageOf(itemProduct, itemDeal); const itemTitle = itemProduct?.title || itemDeal.title || `${itemLabel} Deal`; const itemAvailable = Boolean(itemProduct && Number(itemProduct.stock ?? 0) > 0 && itemRegular > 0); return <Link key={itemDeal.id} href={`/deals/${key}`} className="group overflow-hidden rounded-2xl border border-black/6 bg-white shadow-sm transition hover:-translate-y-0.5"><div className="relative h-44 bg-[#F4F4F1] sm:h-48">{itemImage ? <img src={itemImage} alt={itemTitle} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-[10px] font-bold text-black/25">No image</div>}<div className="absolute left-2 right-2 top-2 flex items-start justify-between gap-1"><span className={`rounded-full border px-2 py-1 text-[7px] font-black uppercase ${statusStyles(itemStatus)}`}>{statusLabel(itemStatus, key)}</span>{itemDiscount > 0 && <span className="rounded-full bg-[#E1352B] px-2 py-1 text-[8px] font-black text-white">-{itemDiscount}%</span>}</div></div><div className="p-3"><p className="text-[8px] font-black uppercase tracking-[0.18em] text-[#E1352B]">{itemLabel} Deal</p><p className="mt-1 line-clamp-2 min-h-[36px] text-sm font-black">{itemTitle}</p><div className="mt-2 flex flex-wrap items-baseline gap-2"><span className="text-[10px] font-bold text-black/40">Regular</span><span className="text-xs text-black/35 line-through">Rs. {itemRegular.toLocaleString()}</span></div><div className="mt-1 flex items-baseline gap-2"><span className="text-[10px] font-bold text-[#E1352B]">Deal</span><span className="font-[family-name:var(--font-mono)] text-base font-black text-[#E1352B]">Rs. {itemDealPrice.toLocaleString()}</span></div><button type="button" disabled={!itemAvailable} onClick={(event) => { event.preventDefault(); event.stopPropagation(); addToCart(itemDeal, itemStatus); }} className="mt-3 flex w-full items-center justify-center gap-1.5 rounded-xl bg-[#14140F] px-2 py-2.5 text-[9px] font-black text-white disabled:opacity-35"><ShoppingBag size={13} />{addingId === itemDeal.id ? 'ADDED' : itemStatus === 'live' ? `BUY NOW — Rs. ${itemDealPrice.toLocaleString()}` : `ADD TO CART — Rs. ${itemRegular.toLocaleString()}`}</button></div></Link>; })}</div>}</div></section>
    </div>
  </main>;
}
