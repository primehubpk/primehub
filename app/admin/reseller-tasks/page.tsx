...
import { uploadImageToImgBB } from '@/components/admin/shared';
...
function newTask(): ResellerTask {
  return {
    ...
    title: 'New reseller task',
    description: 'Add a short instruction for the customer.',
    icon: '✓',
    url: '',
    shareText: '',
    reward: 50,
...
export default function ResellerTasksAdminPage() {
  ...
  useEffect(() => onSnapshot(settingsRef, snapshot => {
    ...
    if (Array.isArray(data.resellerTasks)) {
      setTasks(data.resellerTasks.map((task: ResellerTask) => ({ ...DEFAULT_RESELLER_TASKS.find(item => item.id === task.id), ...task })));
    }
  ...
  function updateTask(id: string, patch: Partial<ResellerTask>) {
    setTasks(current => current.map(task => task.id === id ? { ...task, ...patch } : task));
  }

  function removeTask(id: string) {
    if (!window.confirm('Delete this reseller task?')) return;
  ...
  async function save() {
    ...
    try {
      await setDoc(settingsRef, {
        resellerTasks: tasks,
        resellerMonthlyChallenge: challenge,
        resellerVoucherImages: voucherImages,
        resellerTiers: tiers,
      }, { merge: true });
      setSaved(true);
      window.setTimeout(() => setSaved(false), 1800);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Could not save reseller settings.');
    } finally {
      setSaving(false);
    }
  }
  ...
  async function uploadVoucher(id: string, file?: File) {
    if (!file) return;
    setUploadingVoucher(id);
    setMessage('Uploading voucher image...');
    try {
      const url = await uploadImageToImgBB(file);
      setVoucherImages(current => ({ ...current, [id]: url }));
      setMessage('Voucher image ready. Tap Save Changes to publish it.');
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'Voucher upload failed.');
    } finally {
      setUploadingVoucher('');
    }
  }
    ...
    return <main className="min-h-screen bg-[#F4F4F1] text-[#14140F]">
      <section className="sticky top-0 z-20 border-b border-black/8 bg-white/95 px-4 py-3 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3">
          <button type="button" onClick={() => setView('tasks')} className="inline-flex items-center gap-2 rounded-full bg-[#F4F4F1] px-3 py-2 text-[10px] font-black"><ArrowLeft size={13}/> Tasks & Club</button>
          <Link href="/admin/resellers" className="rounded-full bg-[#14140F] px-3 py-2 text-[10px] font-black text-white">Reseller Management</Link>
        </div>
      </section>
      <RewardsManager />
    </main>;
  ...
  return <main className="min-h-screen bg-[#F4F4F1] text-[#14140F]">
    <section className="sticky top-0 z-20 border-b border-black/8 bg-white/95 px-4 py-3 backdrop-blur">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 rounded-2xl bg-[#F4F4F1] p-1.5">
          <button type="button" onClick={() => setView('tasks')} className="rounded-xl bg-white px-3 py-2.5 text-[10px] font-black shadow-sm">Tasks & Club</button>
          <button type="button" onClick={() => setView('rewards')} className="rounded-xl px-3 py-2.5 text-[10px] font-black text-black/55">Wheel & Rewards</button>
        </div>
      </div>
    </section>
    ...
    <section className="mx-auto max-w-6xl px-3 py-4 sm:px-6 sm:py-6">
      <div className="rounded-[26px] bg-[#14140F] p-5 text-white">
        ...
        <div className="mt-4 flex flex-wrap gap-2"><Link href="/admin/resellers" className="rounded-full bg-white/10 px-3 py-2 text-[9px] font-black">Open member management</Link><button type="button" onClick={() => setView('rewards')} className="rounded-full bg-[#E1352B] px-3 py-2 text-[9px] font-black">Open Wheel & Rewards</button></div>
      </div>
      ...
      <section className="mt-4 rounded-[24px] bg-white p-4 shadow-sm sm:p-5">
        <div className="flex items-center justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-widest text-[#E1352B]">Tasks</p><h2 className="mt-1 text-lg font-black">Reseller tasks</h2><p className="mt-1 text-[11px] leading-5 text-black/45">Add, edit, switch off or delete tasks. Guests can start tasks; reward claiming stays behind login.</p></div><button type="button" onClick={() => setTasks(current => [...current, newTask()])} className="inline-flex shrink-0 items-center gap-1 rounded-xl bg-[#14140F] px-3 py-2.5 text-[9px] font-black text-white"><Plus size={13}/> Add</button></div>
        <div className="mt-4 space-y-2">
          {tasks.map((task, index) => <details key={task.id} className="group rounded-2xl border border-black/6 bg-[#F7F7F3] p-3" open={index === 0}>
            <summary className="flex cursor-pointer list-none items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-black">{task.icon || '✓'} {task.title}</p><p className="mt-0.5 text-[9px] text-black/40">{task.reward} reward · {task.active ? 'ON' : 'OFF'}</p></div><span className={`shrink-0 rounded-full px-2.5 py-1 text-[8px] font-black ${task.active ? 'bg-[#DDF5F0] text-[#0F806E]' : 'bg-black/5 text-black/40'}`}>{task.active ? 'ON' : 'OFF'}</span></summary>
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <Field label={task.id === 'refer-reseller' ? 'Destination link (optional)' : 'Task link'}><input value={task.url || ''} onChange={e => updateTask(task.id, { url: e.target.value })} placeholder="https://..." className="admin-input"/>{task.id === 'refer-reseller' ? <small className="mt-1 block text-[9px] leading-4 text-black/40">Recommended: leave empty. PrimeHub automatically creates the customer&apos;s referral link with their reseller/guest ID.</small> : null}</Field>
...
function Field({ label, wide = false, children }: { label: string; wide?: boolean; children: React.ReactNode }) {
  return <label className={`block text-[8px] font-black uppercase tracking-wide text-black/40 ${wide ? 'sm:col-span-2' : ''}`}>{label}{children}</label>;
}
