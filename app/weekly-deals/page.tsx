...
export default function WeeklyDealsPage() {
  ...
  const addToCart = (deal: WeeklyDeal) => {
    ...
    const price = dealPrice > 0 ? dealPrice : regularPrice;
    if (price <= 0) return;
    ...
    addItem({
      ...
      price,
      originalPrice: Number(product.originalPrice || deal.originalPrice || price),
    ...
    <main className="min-h-screen bg-neutral-50 pb-28">
      <div className="mx-auto max-w-6xl px-4 py-6 md:px-6 md:py-8">
        ...
        <header className="mt-5 overflow-hidden rounded-[30px] bg-white p-6 shadow-sm md:p-8">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-black/50">Browse every weekly deal in one place. Today’s live deal appears first, followed by the remaining days.</p>
        </header>
        ...
        {!deals.length ? (
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {deals.map((deal) => {
              ...
              const image = imageOf(product, deal);
              const price = Number(deal.dealPrice || product?.price || 0);
              const original = Number(product?.originalPrice || deal.originalPrice || price);
              ...
              return (
                <article key={deal.id} className={`overflow-hidden rounded-[28px] border bg-white shadow-[0_12px_40px_rgba(0,0,0,0.06)] ${isToday ? 'border-emerald-300 ring-2 ring-emerald-100' : 'border-black/5'}`}>
                  <div className="relative aspect-square bg-[#F4F4F1]">
                    ...
                    <div className="absolute left-3 right-3 top-3 flex items-center justify-between gap-2"><span className={`rounded-full px-2.5 py-1.5 text-[8px] font-black uppercase tracking-wide ${isToday ? 'bg-emerald-500 text-white' : 'bg-white/95 text-black/70'}`}>{isToday ? "TODAY'S DEAL" : `${LABELS[deal.day]} DEAL`}</span>{discount > 0 && <span className="rounded-full bg-[#E1352B] px-2.5 py-1.5 text-[9px] font-black text-white">-{discount}% OFF</span>}</div>
                  </div>
                  <div className="p-4">
                    ...
                    <div className="mt-3 flex items-end gap-2"><span className="font-[family-name:var(--font-mono)] text-xl font-black text-[#E1352B]">Rs. {price.toLocaleString()}</span>{original > price && <span className="pb-0.5 text-xs text-black/35 line-through">Rs. {original.toLocaleString()}</span>}</div>
