...
async function getAuthoritativeQuote(items: any[]) {
  const response = await fetch('/api/orders', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ mode: 'quote', items }),
  });
...
export default function CheckoutPage() {
  ...
  const subtotal = useMemo(
    () => items.reduce((sum: number, item: any) => sum + priceOf(item) * Number(item.quantity || item.qty || 1), 0),
    [items]
  );
  ...
  const placeOrder = async (e: FormEvent) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || 'Order save nahi ho saka.');
      setOrderId(data.orderId);
      clearCart();
    } catch (err) {
      console.error(err);
      setError(err instanceof Error ? err.message : 'Order save nahi ho saka. Please try again.');
    } finally {
      setPlacing(false);
    }
  };
  ...
  const whatsappOrder = async () => {
    ...
    setError('');
    try {
      const quote = await getAuthoritativeQuote(items);
      const lines = quote.items.map((item: any, index: number) =>
        `${index + 1}. ${item.title} × ${item.quantity} — Rs. ${(Number(item.price) * Number(item.quantity)).toLocaleString()}`
      );
      const text = [
        ...
        `Total: Rs. ${Number(quote.subtotal).toLocaleString()}`,
      ].join('\n');
      window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, '_blank', 'noopener,noreferrer');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Unable to verify current prices.');
    }
  };
  ...
  if (!items.length) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-[#F4F4F1] px-4">
        <div className="w-full max-w-md rounded-[30px] bg-white p-7 text-center shadow-xl">
          <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#F4F4F1]"><ShoppingBag size={22} className="text-black/30" /></div>
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-[1fr_360px]">
          <form onSubmit={placeOrder} className="rounded-[28px] bg-white p-5 shadow-sm md:p-7">
            ...
            <div className="mt-6 grid gap-3">
              <label className="block"><span className="mb-1.5 block text-[9px] font-black uppercase tracking-wider text-black/40">Full Name *</span><div className="flex items-center gap-2 rounded-2xl bg-[#F4F4F1] px-3"><User size={15} className="text-black/30" /><input required value={customer.name} onChange={(e) => update('name', e.target.value)} className="w-full bg-transparent py-3.5 text-xs font-bold outline-none" placeholder="Your name" /></div></label>
              <label className="block"><span className="mb-1.5 block text-[9px] font-black uppercase tracking-wider text-black/40">Phone / WhatsApp *</span><div className="flex items-center gap-2 rounded-2xl bg-[#F4F4F1] px-3"><Phone size={15} className="text-black/30" /><input required type="tel" value={customer.phone} onChange={(e) => update('phone', e.target.value)} className="w-full bg-transparent py-3.5 text-xs font-bold outline-none" placeholder="+92 3XX XXXXXXX" /></div></label>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                <label className="block"><span className="mb-1.5 block text-[9px] font-black uppercase tracking-wider text-black/40">City *</span><div className="flex items-center gap-2 rounded-2xl bg-[#F4F4F1] px-3"><MapPin size={15} className="text-black/30" /><input required value={customer.city} onChange={(e) => update('city', e.target.value)} className="w-full bg-transparent py-3.5 text-xs font-bold outline-none" placeholder="City" /></div></label>
          ...
          <aside className="h-fit rounded-[28px] bg-white p-5 shadow-sm md:sticky md:top-4">
            ...
            <div className="mt-4 space-y-3">
              {items.map((item: any, index: number) => {
                ...
                return <div key={`${item.id || item.productId || index}`} className="flex gap-3"><div className="h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-[#F4F4F1]">{imageOf(item) && <img src={imageOf(item)} alt="" className="h-full w-full object-cover" />}</div><div className="min-w-0 flex-1"><p className="line-clamp-2 text-[10px] font-black">{titleOf(item)}</p><p className="mt-1 text-[9px] font-bold text-black/40">Qty {qty}</p></div><p className="font-[family-name:var(--font-mono)] text-[10px] font-black">Rs. {(priceOf(item) * qty).toLocaleString()}</p></div>;
              })}
            </div>
