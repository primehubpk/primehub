...
import { ArrowLeft, Heart, PackageCheck, RefreshCcw, ShieldCheck, ShoppingBag, Truck } from 'lucide-react';
...
export default function ProductDetailPage() {
  ...
  const { product, weeklyProducts, loading, failed, activeImage, quantity, wished, videoOpen, added, nowTick, images, regularPrice, productOriginal, stock, rating, reviews, weeklyDeals, currentDeal, liveDeal, dealPrice, normalForDeal, savingsAmount, currentPrice, maxQuantity, stockProgress, setActiveImage, setQuantity, setWished, setVideoOpen, addProduct, orderNow, buyWhatsApp } = model;
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid gap-5 px-3 md:grid-cols-[1.04fr_.96fr] md:px-5 md:pt-3">
        ...
        <section className="rounded-[30px] border border-black/7 bg-white p-4 shadow-sm sm:p-6 md:p-7">
          <div className="overflow-hidden rounded-2xl border border-[#E1352B]/15 bg-[#E1352B]/[0.04]"><div className="flex items-center justify-between gap-3 px-3 py-2.5"><span className="flex items-center gap-2 text-[10px] font-black text-[#E1352B]"><span aria-hidden="true">⚡</span>{stock > 0 ? `🔥 Selling Fast! Only ${stock} left in stock.` : 'Stock is limited — check availability before ordering.'}</span><span className="shrink-0 text-[8px] font-black uppercase tracking-wider text-black/35">Limited</span></div>{stock > 0 && <div className="h-1.5 bg-black/5"><div className="h-full rounded-full bg-[#E1352B] transition-[width] duration-500" style={{ width: `${stockProgress}%` }} /></div>}</div>

          <ProductPricing product={product} rating={rating} reviews={reviews} currentDeal={currentDeal} liveDeal={liveDeal} dealPrice={dealPrice} regularPrice={regularPrice} productOriginal={productOriginal} normalForDeal={normalForDeal} savingsAmount={savingsAmount} afterPricing={<>
