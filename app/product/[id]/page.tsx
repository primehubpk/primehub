    ...
    <main className="min-h-screen bg-[#F4F4F1] pb-28">
      ...
      <div className="mx-auto max-w-6xl">
        <header className="sticky top-0 z-30 flex items-center justify-between bg-[#F4F4F1]/92 px-3 py-3 backdrop-blur md:px-5">
          <Link href="/" className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm" aria-label="Back to home">
            <ArrowLeft size={17} />
          </Link>
          <span className="text-[10px] font-black uppercase tracking-[0.24em] text-black/40">PrimeHub Product</span>
          <button type="button" onClick={() => setWished((v) => !v)} className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm" aria-label={wished ? 'Remove from wishlist' : 'Add to wishlist'}>
            <Heart size={17} className={wished ? 'text-[#E1352B]' : 'text-[#14140F]'} fill={wished ? 'currentColor' : 'none'} />
          </button>
        </header>
        ...
        <div className="grid gap-5 px-3 md:grid-cols-[1.04fr_.96fr] md:px-5 md:pt-3">
          <section className="overflow-hidden rounded-[30px] border border-black/7 bg-white shadow-sm md:sticky md:top-[66px] md:self-start">
            <div className="relative aspect-square overflow-hidden bg-[#F4F4F1] md:aspect-[4/3]">
              {displayImage ? <img src={displayImage} alt={titleOf(product)} className="h-full w-full object-cover" /> : <div className="flex h-full items-center justify-center text-sm font-bold text-black/25">No product image</div>}
              ...
              {images.length > 1 && (
                <>
                  <button type="button" onClick={() => setActiveImage((i) => (i - 1 + images.length) % images.length)} className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/92 shadow-md" aria-label="Previous image">
                    <ChevronLeft size={18} />
                  </button>
                  <button type="button" onClick={() => setActiveImage((i) => (i + 1) % images.length)} className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full bg-white/92 shadow-md" aria-label="Next image">
                    <ChevronRight size={18} />
                  </button>
                </>
              )}
              {videoOf(product) && (
                <button type="button" onClick={() => setVideoOpen(true)} className="absolute bottom-3 left-3 flex items-center gap-1.5 rounded-full bg-white/95 px-3 py-2 text-[10px] font-black shadow-md">
            ...
            {images.length > 1 && (
              <div className="flex gap-2 overflow-x-auto p-3">
                {images.map((image, index) => (
                  <button type="button" key={`${image}-${index}`} onClick={() => setActiveImage(index)} className={`h-16 w-16 shrink-0 overflow-hidden rounded-xl border-2 ${index === activeImage ? 'border-[#E1352B]' : 'border-transparent'}`} aria-label={`View image ${index + 1}`}>
          ...
          <section className="rounded-[30px] border border-black/7 bg-white p-4 shadow-sm sm:p-6 md:p-7">
            {currentDeal && timing && nowTick !== null && (liveDeal ? (
              <div className="live-deal-celebration relative overflow-hidden rounded-[24px] bg-gradient-to-r from-[#8a4b00] via-[#FFD166] to-[#0F6A5F] p-[2px] shadow-lg">
                <div className="rounded-[22px] bg-gradient-to-br from-[#2b1600] via-[#14140F] to-[#063b35] p-4 text-white sm:p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <div className="mb-1 text-[8px] font-black uppercase tracking-[0.14em] text-[#FFD166]">🎉 MEGA CELEBRATION SALE — LOWEST PRICE EVER!</div>
                      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-white/80"><span className="h-2 w-2 animate-pulse rounded-full bg-[#FFB020]" />Live Deal <span aria-hidden="true">🎉 🎆 💥</span></div>
                      <p className="mt-1 text-xl font-black sm:text-2xl">⚡ LIVE TODAY'S DEAL</p>
                    </div>
                    <div className="rounded-2xl bg-white/10 px-4 py-3 text-center ring-1 ring-[#FFD166]/30">
                      <p className="text-[8px] font-black uppercase tracking-[0.18em] text-white/60">Ends in</p>
                      <p className="mt-1 font-[family-name:var(--font-mono)] text-xl font-black tracking-tight">{bannerCountdown}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-[24px] border border-black/8 bg-[#F4F4F1] p-4 sm:p-5">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-[#0F6A5F]"><LockKeyhole size={13} />Weekly Deal</div>
                    <p className="mt-1 text-xl font-black sm:text-2xl">🔒 {WEEKDAY_LABELS[currentDeal.day]} DEAL</p>
                  </div>
                  <div className="rounded-2xl bg-white px-4 py-3 text-center shadow-sm">
                    <p className="text-[8px] font-black uppercase tracking-[0.18em] text-black/40">Unlocks in</p>
                    <p className="mt-1 font-[family-name:var(--font-mono)] text-lg font-black">{bannerCountdown}</p>
                  </div>
                </div>
            ...
            {currentDeal && !liveDeal ? (
              <div className="mt-5 overflow-hidden rounded-[24px] border border-black/10 bg-white shadow-sm">
                <div className="border-b border-black/8 bg-[#FCFCFA] p-4 sm:p-5">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      <p className="text-[9px] font-black uppercase tracking-[0.16em] text-[#0F6A5F]">🟢 Buy Today (Regular Store Price)</p>
                      <p className="mt-2 font-mono text-3xl font-black text-[#14140F] sm:text-4xl">{money(regularPrice)}</p>
                    </div>
                ...
                <div className="border-2 border-[#D4A72C]/40 bg-[#FFF9E8] p-4 sm:p-5">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    ...
                    {dealDiscount(currentDeal.dealPrice, regularPrice) > 0 && (
              ...
              <div className="mt-5 rounded-[24px] border border-black/7 bg-[#FCFCFA] p-4 sm:p-5">
                ...
                <div className="mt-1 flex items-baseline gap-2">
                  <span className="font-mono text-3xl font-black text-[#14140F] sm:text-4xl">{money(currentPrice)}</span>
                  {productOriginal > currentPrice && (
                    <span className="font-mono text-sm text-black/35 line-through">{money(productOriginal)}</span>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="mt-4 overflow-hidden rounded-2xl border border-[#E1352B]/15 bg-[#E1352B]/[0.04]">
              <div className="flex items-center justify-between gap-3 px-3 py-2.5">
                <span className="flex items-center gap-2 text-[10px] font-black text-[#E1352B]"><Zap size={13} fill="currentColor" />{stock > 0 ? `🔥 Selling Fast! Only ${stock || 10} left in stock.` : 'Stock is limited — check availability before ordering.'}</span>
                <span className="shrink-0 text-[8px] font-black uppercase tracking-wider text-black/35">Limited</span>
              </div>
              {stock > 0 && <div className="h-1.5 bg-black/5"><div className="h-full rounded-full bg-[#E1352B] transition-[width] duration-500" style={{ width: `${stockProgress}%` }} /></div>}
            </div>

            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
              {[{ icon: <PackageCheck size={16} />, label: 'Cash on Delivery' }, { icon: <Truck size={16} />, label: 'Fast Delivery' }, { icon: <ShieldCheck size={16} />, label: '100% Quality Guaranteed' }, { icon: <RefreshCcw size={16} />, label: 'Easy Returns' }].map((badge) => (
                <div key={badge.label} className="flex min-h-[68px] flex-col items-center justify-center gap-1 rounded-2xl bg-[#F4F4F1] px-2 text-center">
                  <span className="text-[#0F6A5F]">{badge.icon}</span>
                  <span className="text-[8px] font-black leading-3 text-black/55">{badge.label}</span>
                </div>
              ))}
            </div>
            ...
            {product?.description && (
              <div className="mt-5">
                <h2 className="text-xs font-black uppercase tracking-[0.15em]">About this product</h2>
                <p className="mt-2 whitespace-pre-line text-xs leading-5 text-black/55">{product.description}</p>
              </div>
            )}

            <div className="mt-5 flex items-center justify-between rounded-2xl border border-black/8 p-2">
              ...
              <div className="flex items-center rounded-xl bg-[#F4F4F1]">
                ...
                <button type="button" onClick={() => setQuantity((q) => maxQuantity ? Math.min(maxQuantity, q + 1) : q + 1)} className="flex h-10 w-10 items-center justify-center" aria-label="Increase quantity"><Plus size={14} /></button>
              </div>
            </div>

            <div className="mt-4 grid gap-2.5">
              <button type="button" onClick={orderNow} disabled={stock === 0 || currentPrice <= 0} className={`live-deal-cta flex items-center justify-center gap-2 rounded-2xl bg-[#E1352B] py-4 text-xs font-black text-white shadow-lg shadow-[#E1352B]/20 transition hover:brightness-95 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-45 ${liveDeal ? 'live-deal-cta-glow' : ''}`}>
                <Zap size={16} fill="currentColor" />{stock === 0 ? 'Out of Stock' : liveDeal ? '🎉 CLAIM DEAL NOW' : 'BUY NOW / ORDER NOW'}
              </button>
              <button type="button" onClick={addProduct} disabled={stock === 0 || currentPrice <= 0} className="flex items-center justify-center gap-2 rounded-2xl bg-[#14140F] py-4 text-xs font-black text-white transition hover:bg-[#0F6A5F] active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-45">
                {added ? <Check size={16} /> : <ShoppingCart size={16} />}{added ? 'Added to Cart' : 'ADD TO CART'}
              </button>
              <button type="button" onClick={buyWhatsApp} disabled={stock === 0 || currentPrice <= 0} className="flex items-center justify-center gap-2 rounded-2xl border border-[#0F6A5F]/20 bg-[#0F6A5F]/8 py-4 text-xs font-black text-[#0F6A5F] transition hover:bg-[#0F6A5F]/15 active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-45">
      ...
      {videoOpen && product && videoOf(product) && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 p-4" onClick={() => setVideoOpen(false)}>
          <div className="relative w-full max-w-lg" onClick={(e) => e.stopPropagation()}>
            <button type="button" onClick={() => setVideoOpen(false)} className="absolute -right-1 -top-12 flex h-9 w-9 items-center justify-center rounded-full bg-white" aria-label="Close video">
              <X size={16} />
            </button>
