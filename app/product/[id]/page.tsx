...
export default function ProductDetailPage() {
  ...
  const { product, weeklyProducts, loading, failed, activeImage, quantity, wished, videoOpen, added, nowTick, images, regularPrice, productOriginal, stock, rating, reviews, weeklyDeals, currentDeal, timing, liveDeal, dealPrice, normalForDeal, savingsAmount, savingsPercent, countdown, currentPrice, maxQuantity, stockProgress, bannerCountdown, setActiveImage, setQuantity, setWished, setVideoOpen, addProduct, orderNow, buyWhatsApp } = model;
  ...
  return <main className="min-h-screen bg-[#F4F4F1] pb-28">
    ...
    <div className="mx-auto max-w-6xl">
      ...
      <div className="grid gap-5 px-3 md:grid-cols-[1.04fr_.96fr] md:px-5 md:pt-3">
        <ProductHero product={product} images={images} activeImage={activeImage} savingsPercent={savingsPercent} liveDeal={liveDeal} onImageChange={(updater) => setActiveImage(updater)} onVideoOpen={() => setVideoOpen(true)} />
        <section className="rounded-[30px] border border-black/7 bg-white p-4 shadow-sm sm:p-6 md:p-7">
          ...
          <ProductPricing product={product} rating={rating} reviews={reviews} currentDeal={currentDeal} liveDeal={liveDeal} dealPrice={dealPrice} regularPrice={regularPrice} productOriginal={productOriginal} normalForDeal={normalForDeal} savingsAmount={savingsAmount} savingsPercent={savingsPercent} stock={stock} stockProgress={stockProgress} />
