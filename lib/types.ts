...
export type Weekday='sunday'|'monday'|'tuesday'|'thursday'|'friday'|'saturday'|'wednesday';
...
export interface DailyDeal { productId:string; productIds?:string[]; imageUrl:string; imageUrls?:string[]; galleryImages?:string[]; titles?:string[]; categoryIds?:string[]; originalPrices?:number[]; dealPrices?:number[]; rotationStartedAt?:string; title:string; originalPrice:number; dealPrice:number; startAt:string; endAt:string; buttonText:string; buttonLink:string; active:boolean; }
...
export interface SiteSettings { resellerHomeEnabled?:boolean; freeDelivery?:FreeDeliverySettings; priceBuckets?:PriceBucket[]; weeklyDeals?:WeeklyDeal[]; dailyDeal?:DailyDeal; youtubeGuide?:YouTubeGuideSettings; youtubeGuideUrl?:string; youtubeGuideTitle?:string; youtubeGuidePreviewSeconds?:number; youtubeGuideHomeEnabled?:boolean; policies?:{privacyPolicy?:PolicyPageContent;terms?:PolicyPageContent;returnPolicy?:PolicyPageContent}; contact?:ContactSettings; announcementText:string; whatsappNumber:string; freeShippingCount:number; heroTitle:string; heroDiscountText:string; heroCountdownEndTime:string; heroImageUrl:string; heroButtonText:string; heroButtonLink:string; }
