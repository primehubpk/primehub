...
async function knowledge(payload: Record<string, any>) {
  ...
  const sources = ranked.slice(0, 5).map(({ p }) => ({ title: String(p.title || p.key || topic), url: String(p.url || ''), text: String(p.text_excerpt || '').slice(0, 3000) })); const best = sources[0]; const value = { type: 'knowledge', query: topic, title: best.title, url: best.url, text: best.text, sources, cached: false }; await writeCache(key, value); return value;
}
