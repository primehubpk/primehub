...
export type SalarWorkerJob = 'catalogue' | 'knowledge' | 'vision' | 'order';
...
function scoreText(value: unknown, queryTerms: string[]) {
  const hay = norm(value); if (!hay) return 0;
  let score = 0;
  for (const term of queryTerms) { if (!term) continue; if (hay === term) score += 100; else if (hay.startsWith(term)) score += 60; else if (hay.includes(term)) score += 30; }
  return score;
}
...
async function readFreshCache(key: string) {
  ...
  const data: any = snap.data() || {}; const created = Date.parse(String(data.created_at || ''));
...
export async function clearSalarCache() {
  const db = getAdminDb(); const snap = await db.collection('salar_cache').get();
  for (let i = 0; i < snap.docs.length; i += 400) { const batch = db.batch(); snap.docs.slice(i, i + 400).forEach((doc) => batch.delete(doc.ref)); await batch.commit(); }
}

function mappedProduct(p: any) {
  return {
    id: String(p.source_id || p.id), name: String(p.name || p.title || p.id), price: Number(p.price) || 0,
    image_url: list(p.image_urls)[0] || p.imageUrl || null, size: list(p.sizes).join(', ') || p.size || null,
...
async function catalogue(payload: Record<string, any>) {
  ...
  const [collectionSnap, productSnap] = await Promise.all([db.collection('salar_index_collections').get(), db.collection('salar_index_products').get()]);
  ...
  if (!requestedId && !requestedCollection) {
    ...
    const matched = collections.map((c) => {
  ...
  const key = `cat:${String(collection.id)}|${norm(q)}|${limit}`; const cached = await readFreshCache(key); if (cached) return { ...cached, cached: true };
  ...
  await writeCache(key, value); return value;
}

const TOPIC_ALIASES: Record<string, string[]> = {
  reseller_club: ['reseller_club', 'reseller club', 'reseller'], prime_skill: ['prime_skill', 'prime skill', 'skills'], shopping: ['shopping', 'shop'],
...
async function knowledge(payload: Record<string, any>) {
  ...
  const snap = await getAdminDb().collection('salar_index_pages').get(); const pages = snap.docs.map((doc) => ({ id: doc.id, ...doc.data() } as any));
  if (!pages.length) return { found: false, reason: 'Knowledge index is empty. Run Catalogue Refresh first.' };
  const aliases = TOPIC_ALIASES[norm(topic).replace(/ /g, '_')] || [topic]; const qTerms = [...new Set(aliases.flatMap(terms))];
  ...
  const best = ranked[0].p; const value = { title: String(best.title || best.key || topic), url: String(best.url || ''), text: String(best.text_excerpt || ''), cached: false };
  await writeCache(key, value); return value;
}
...
async function vision(payload: Record<string, any>) {
  const imageUrl = String(payload.imageUrl || '').trim(); const question = String(payload.question || 'Is image ke bare mein batayein.').trim().slice(0, 1200);
  if (!imageUrl || !isR2PublicUrl(imageUrl)) return { ok: false, reason: 'Vision requires a trusted PrimeHub R2 image URL.' };
  const candidates = list(payload.productCandidates).slice(0, 30).map((item: any) => ({ id: String(item?.id || ''), name: String(item?.name || ''), image_url: String(item?.image_url || ''), size: item?.size ? String(item.size) : null, material: item?.material ? String(item.material) : null })).filter((item) => item.id);
  const prompt = `You inspect a customer image for PrimeHub Mall. Answer the question using the image. If it visually matches one supplied candidate, prefer that site product id. Never invent or estimate a price. Return JSON only: {"matchProductId":string|null,"confidence":number,"description":string,"answer":string}. Candidate products: ${JSON.stringify(candidates)}. Customer question: ${question}`;
  const completion = await visionCompletion({ imageUrl, prompt }); if (!completion.ok) return { ok: false, reason: 'Vision providers unavailable.' };
  ...
  const proposed = String(parsed.matchProductId || '').trim(); const matchProductId = proposed && candidates.some((candidate) => candidate.id === proposed) ? proposed : null;
  return { ok: true, matchProductId, confidence: Number.isFinite(Number(parsed.confidence)) ? Number(parsed.confidence) : null, description: String(parsed.description || '').slice(0, 1200), answer: String(parsed.answer || parsed.description || '').slice(0, 1200) };
}
