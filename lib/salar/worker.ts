...
import { runSalarOrder } from '@/lib/salar/orderService';
import { moderateSalarText } from '@/lib/salar/moderation';

export type SalarWorkerJob = 'catalogue' | 'knowledge' | 'vision' | 'order' | 'moderate';
...
const COMMON_SYNONYMS: Record<string, string[]> = {
  ...
  reseller: ['reseller', 'reseller club'], skill: ['skill', 'prime skill', 'skills'],
};

function norm(value: unknown) { return String(value ?? '').toLowerCase().replace(/[^a-z0-9\u0600-\u06ff]+/g, ' ').replace(/\s+/g, ' ').trim(); }
function list(value: unknown) { return Array.isArray(value) ? value : []; }
function terms(query: string) { const q = norm(query); const base = q.split(' ').filter(Boolean); const extra = base.flatMap((token) => COMMON_SYNONYMS[token] || []); return [...new Set([q, ...base, ...extra.map(norm)].filter(Boolean))]; }
function scoreText(value: unknown, queryTerms: string[]) { const hay = norm(value); if (!hay) return 0; let score = 0; for (const term of queryTerms) { if (!term) continue; if (hay === term) score += 100; else if (hay.startsWith(term)) score += 60; else if (hay.includes(term)) score += 30; } return score; }
...
async function catalogue(payload: Record<string, any>) {
  ...
  await writeCache(requestKey, value); return value;
}
...
async function knowledge(payload: Record<string, any>) {
  const topic = String(payload.query || payload.topic || '').trim(); if (!topic) return { found: false, reason: 'Provide a website question or topic.' };
  const key = `know:v2:${norm(topic)}`; const state = await indexState(); if (state.stale) return refreshRequired('Knowledge index is stale or missing.');
  ...
  const aliases = TOPIC_ALIASES[norm(topic).replace(/ /g, '_')] || [topic]; const qTerms = [...new Set(aliases.flatMap(terms))]; const ranked = pages.map((p) => ({ p, score: Math.max(scoreText(p.key, qTerms) * 2, scoreText(p.title, qTerms) * 2, scoreText(p.url, qTerms), scoreText(p.text_excerpt, qTerms)) })).filter((x) => x.score > 0).sort((a, b) => b.score - a.score);
  if (!ranked.length) return { found: false, reason: `No indexed page matched "${topic}".` };
  const sources = ranked.slice(0, 3).map(({ p }) => ({ title: String(p.title || p.key || topic), url: String(p.url || ''), text: String(p.text_excerpt || '').slice(0, 1600) })); const best = sources[0]; const value = { type: 'knowledge', query: topic, title: best.title, url: best.url, text: best.text, sources, cached: false }; await writeCache(key, value); return value;
}
function parseVisionJson(text: string) { const cleaned = text.trim().replace(/^```(?:json)?\s*/i, '').replace(/\s*```$/i, ''); try { const parsed = JSON.parse(cleaned); return parsed && typeof parsed === 'object' ? parsed : null; } catch { return null; } }
async function vision(payload: Record<string, any>) { const imageUrl = String(payload.imageUrl || '').trim(); const question = String(payload.question || 'Is image ke bare mein batayein.').trim().slice(0, 1200); if (!imageUrl || !isR2PublicUrl(imageUrl)) return { ok: false, reason: 'Vision requires a trusted PrimeHub R2 image URL.' }; const candidates = list(payload.productCandidates).slice(0, 30).map((item: any) => ({ id: String(item?.id || ''), name: String(item?.name || ''), image_url: String(item?.image_url || ''), size: item?.size ? String(item.size) : null, material: item?.material ? String(item.material) : null })).filter((item) => item.id); const prompt = `You inspect a customer image for PrimeHub Mall. Answer the question using the image. If it visually matches one supplied candidate, prefer that site product id. Never invent or estimate a price. Return JSON only: {"matchProductId":string|null,"confidence":number,"description":string,"answer":string}. Candidate products: ${JSON.stringify(candidates)}. Customer question: ${question}`; const completion = await visionCompletion({ imageUrl, prompt }); if (!completion.ok) return { ok: false, reason: 'Vision providers unavailable.' }; const parsed: any = parseVisionJson(completion.text); if (!parsed) return { ok: true, matchProductId: null, confidence: null, description: completion.text.slice(0, 1200), answer: completion.text.slice(0, 1200) }; const proposed = String(parsed.matchProductId || '').trim(); const matchProductId = proposed && candidates.some((candidate) => candidate.id === proposed) ? proposed : null; return { ok: true, matchProductId, confidence: Number.isFinite(Number(parsed.confidence)) ? Number(parsed.confidence) : null, description: String(parsed.description || '').slice(0, 1200), answer: String(parsed.answer || parsed.description || '').slice(0, 1200) }; }
...
export async function runWorker({ job, payload = {}, conversationId = null }: SalarWorkerInput) {
  ...
  switch (job) {
    ...
    case 'order': result = await runSalarOrder({ conversationId: String(payload.conversationId || conversationId || ''), action: payload.action === 'commit' ? 'commit' : 'quote' }); break;
    case 'moderate': result = moderateSalarText(payload.text); break;
    default: result = { found: false, reason: `Unsupported worker job: ${String(job || '')}` };
  }
  console.info('[salar-worker]', { job, conversationId: conversationId || null, type: result?.type || null, found: result?.found !== false && result?.ok !== false, cached: result?.cached === true, flagged: job === 'moderate' ? result?.flag === true : undefined });
  return result;
}
