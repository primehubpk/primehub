...
async function knowledge(payload: Record<string, any>) {
  ...
  const best = ranked[0].p; const value = { title: String(best.title || best.key || topic), url: String(best.url || ''), text: String(best.text_excerpt || '').slice(0, 1800), cached: false }; await writeCache(key, value); return value;
}
