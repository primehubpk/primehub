...
type Interpretation = {
  ...
  needsSelectedImageVision: boolean;
};
...
function parseInterpretation(raw: string): Interpretation | null {
  ...
  return {
    ...
    needsSelectedImageVision: parsed.needsSelectedImageVision === true,
  };
}
...
async function catalogueReferenceVision(
  ...
  if (!exactProductIds.length) return null;
  ...
  const jobs: Array<Promise<ProviderText | null>> = [];
    ...
    for (const imageUrl of imageUrls) {
      jobs.push((async () => {
        try {
          ...
          return await analyzeImage({ mimeType, base64: Buffer.from(bytes).toString('base64') }, message);
        } catch (error) {
          console.warn('Salar selected catalogue image could not be inspected.', error instanceof Error ? error.message : 'unknown');
          return null;
        }
      })());
  ...
  const inspected = (await Promise.all(jobs)).filter(Boolean) as ProviderText[];
  if (!inspected.length) return null;
  const descriptions = [...new Set(inspected.map((item) => item.text).filter(Boolean))];
  return {
    text: descriptions.map((description, index) => `Image ${index + 1}: ${description}`).join('\n'),
    provider: inspected[0].provider,
    model: inspected[0].model,
  };
}
...
async function interpretCustomer(input: {
  ...
  const protocol = [
    ...
    'Set needsSelectedImageVision=true only when answering this turn requires inspecting the visible contents of an exact product image already selected or referenced in the conversation. Catalogue fields such as named variants alone do not require vision.',
    'Return JSON only: {"searchText":"...","intentSummary":"...","catalogueMode":"none|products|categories","resultScope":"focused|all","shoppingMode":"retail|wholesale","requirements":[{"name":"...","value":"..."}],"requiredTerms":["..."],"continuation":false,"needsSelectedImageVision":false}.',
  ].join(' ');
...
function parseFinalDecision(raw: string): FinalDecision | null {
  ...
  const orderAction: OrderAction = rawOrderAction === 'place' ? 'place' : rawOrderAction === 'draft' ? 'draft' : 'none';
  ...
  }).filter(([, value]) => value)) as OrderCustomerDraft;
  if (!reply && display === 'none' && orderAction === 'none') return null;
  return { reply, display, productIds, categoryIds, showAllMatches, orderAction, orderProductIds, orderCustomer };
}
...
function buildFinalSystem(input: {
  ...
  const protocol = [
    ...
    'Set orderAction=place only when Admin Salesman Training says the order may now be submitted and the customer has completed every prerequisite defined there. Never infer a payment requirement, amount, confirmation rule or sequence yourself. For draft or place, return all remembered exact order product ids and extract any customer-provided name, phone, email, city and address into orderCustomer; leave unknown fields empty.',
    ...
    'Return exactly one JSON object with no markdown: {"reply":"natural customer-facing reply","display":"none|categories|products|product_images","showAllMatches":false,"productIds":["id"],"categoryIds":["id"],"orderAction":"none|draft|place","orderProductIds":["id"],"orderCustomer":{"name":"","phone":"","email":"","city":"","address":""}}.',
...
export async function answerWithModelDrivenSalar(input: {
  ...
  const understood = await interpretCustomer({
    ...
    history,
    imageDescription: uploadedVision?.text,
    adminInstructions: state.instructions,
  });
  const interpretation = understood.interpretation;
  const selectedProductVision = !input.image && interpretation.needsSelectedImageVision
  ...
  console.info('Salar first-model understanding', {
    ...
    needsSelectedImageVision: interpretation.needsSelectedImageVision,
  ...
  let lastUnstructured: ProviderText | null = null;
  let lastError: unknown = null;
  const invalidFormatProviders = new Set<ProviderName>();
  ...
  if (!decision || !finalProvider) {
    if (lastUnstructured) {
      decision = { reply: cleanText(lastUnstructured.text, 6000), display: 'none', productIds: [], categoryIds: [], showAllMatches: false, orderAction: 'none', orderProductIds: [], orderCustomer: {} };
      finalProvider = lastUnstructured;
    } else {
      throw new Error(`No working Salar AI provider.${lastError instanceof Error ? ` ${lastError.message}` : ''}`);
    }
  }
  ...
  return {
    ...
    imageUnderstanding: vision?.text || '',
