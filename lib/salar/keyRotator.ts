...
function shouldRotate(status:number,payload:string){return [401,403,429].includes(status)||/quota|billing|resource[_\s-]?exhausted|rate[_\s-]?limit|too many requests|api.?key|unauthorized|forbidden|authentication/i.test(payload);}
