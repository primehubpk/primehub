...
function slot(provider:Provider,index:number,model='*'){return `${provider}:${model}:${index}`;}
function markDead(provider:Provider,index:number,model='*'){deadMap().set(slot(provider,index,model),Date.now()+DEAD_KEY_TTL_MS);}
function clearDead(provider:Provider,index:number,model='*'){deadMap().delete(slot(provider,index,model));}
function coolingSlot(provider:Provider,index:number,model:string){const key=slot(provider,index,model);const until=deadMap().get(key)||0;if(until<=Date.now()){deadMap().delete(key);return false;}return true;}
function isCooling(provider:Provider,index:number,model:string){return coolingSlot(provider,index,'*')||coolingSlot(provider,index,model);}
...
function remember(provider:Provider,index:number,model:string){globalThis.__salarLastGood={provider,index,at:Date.now()};clearDead(provider,index,'*');clearDead(provider,index,model);}
function keyOrder(provider:Provider,length:number,model:string){if(length<=0)return[];const last=currentLastGood();const start=last?.provider===provider&&last.index>=0&&last.index<length?last.index:0;return Array.from({length},(_,o)=>(start+o)%length).filter((index)=>!isCooling(provider,index,model));}
...
async function attemptProvider(provider:Provider,keys:string[],models:string[],messages:SalarLlmMessage[],purpose:SalarPurpose,maxTokens:number,tools:SalarToolDefinition[]):Promise<CompletionResult|null>{
  ...
  for(const model of models){
    const order=keyOrder(provider,keys.length,model);
    ...
    for(const index of order){
      try{
        ...
        if(error instanceof RotatableProviderError){
          ...
          if(error.keyRejected){markDead(provider,index,error.status===429?model:'*');continue;}
          break;
        }
...
async function runCompletion(messages:SalarLlmMessage[],purpose:SalarPurpose,maxTokens:number,tools:SalarToolDefinition[]):Promise<CompletionResult>{
  const providers:Array<{provider:Provider;keys:string[];models:string[]}>= [
    {provider:'groq',keys:envList('GROQ_API_KEYS','GROQ_API_KEY'),models:modelList(['GROQ_MODELS','GROQ_MODEL'],['openai/gpt-oss-120b','openai/gpt-oss-20b','llama-3.1-8b-instant','llama-3.3-70b-versatile'])},
