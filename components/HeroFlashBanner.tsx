...
function hasProductVariants(product?: Product): boolean {
  ...
  return Boolean(
    ...
    (Array.isArray(product.variantMatrix) &&
      product.variantMatrix.length > 0) ||
...
type BigDealFields = NonNullable<
  ReturnType<typeof useSettings>["settings"]["dailyDeal"]
> & {
  dealPrice?: number | string;
  normalPrice?: number | string;
  originalPrice?: number | string;
...
export default function HeroFlashBanner({
  ...
  const countdown = useMemo(() => {
    ...
    return countdownParts(
        ...
        ? end - nowTick
        : fallback.days * 86400000 +
            fallback.hours * 3600000 +
            fallback.minutes * 60000 +
  ...
  function addDealToCart(deal: NonNullable<typeof weeklyDeals>[number]) {
    ...
    const dealDay = deal.day
      ? `${deal.day.charAt(0).toUpperCase()}${deal.day.slice(1)}`
      : undefined;
    const price = getEffectivePrice(
      { price: normalPrice, dealPrice: specialPrice, dealDay },
      new Date(nowTick || Date.now()),
    );
    const isLive =
      todayKey === deal.day && specialPrice > 0 && price === specialPrice;
    if (
      !deal.productId ||
      price <= 0 ||
      Number((product as ProductDealFields | undefined)?.stock ?? 1) <= 0
    )
      return;
    ...
    const productWithDealPrice = {
      ...
      price,
      originalPrice: isLive
        ? Number(product?.originalPrice || deal.originalPrice || price)
        : normalPrice,
      image,
      imageUrl: image,
    } as Product;
    ...
    if (
      hasProductVariants(productWithDealPrice) &&
  ...
  function addBigDealToCart() {
    ...
    const currentPrice = Number(
      bigDeal.dealPrice || productData?.dealPrice || productData?.price || 0,
    );
    const normalPrice = Number(
      bigDeal.normalPrice ||
    ...
    const stock = Number(
      productData?.stock ?? productData?.quantity ?? bigDeal.stock ?? 0,
    );
    if (currentPrice <= 0 || stock <= 0) return;

    const image = normalizeImageUrl(
      productData?.imageUrl || bigDeal.imageUrl || "",
    );
    const productWithDealPrice = {
      ...product,
  ...
  if (homeLayout) {
    const product = bigDeal?.productId
      ? (products[bigDeal.productId] as ProductDealFields | undefined)
      : undefined;
    const price = Number(
      bigDeal?.dealPrice || product?.dealPrice || product?.price || 0,
    );
    const regularPrice = Number(
      bigDeal?.normalPrice ||
        bigDeal?.originalPrice ||
        product?.normalPrice ||
            ...
            {orderedDays.map(({ key, Icon }) => {
              const deal = weeklyDeals.find(
                (d) =>
                  d.day === key &&
                  d.active !== false &&
                  Number(d.dealPrice) > 0,
              );
              const dealProduct = deal
                ? (products[deal.productId] as ProductDealFields | undefined)
                : undefined;
              const dealPrice = Number(deal?.dealPrice || 0);
              const weeklyRegular = Number(
                deal?.normalPrice ||
                  deal?.originalPrice ||
                  dealProduct?.normalPrice ||
                  dealProduct?.price ||
                  dealPrice,
              );
              const saving = Math.max(0, weeklyRegular - dealPrice);
              const dealImage = normalizeImageUrl(
                deal?.imageUrl ||
                  (deal ? products[deal.productId]?.imageUrl : "") ||
                  "",
              );
              ...
              return (
                <article
                  key={key}
                  className={`home-week-card ${isLive ? "is-live" : ""}`}
                >
                  <Link
                    className="home-week-link"
                    href={deal ? `/product/${deal.productId}` : "/weekly-deals"}
                  >
                    ...
                    <span className="home-week-image">
                      {dealImage ? (
                        <Image
                          ...
                          alt={deal?.title || key}
                          fill
                          sizes="(max-width: 600px) 80px, 170px"
                    ...
                    {deal ? (
                      <span className="home-week-pricing">
                        <small>
                          Deal <b>Rs. {dealPrice.toLocaleString("en-PK")}</b>
                        </small>
                        <small>
                          Regular{" "}
                          <s>Rs. {weeklyRegular.toLocaleString("en-PK")}</s>
                        </small>
                        {saving > 0 ? (
                          <em>Save Rs. {saving.toLocaleString("en-PK")}</em>
                        ) : null}
                      </span>
                    ) : (
                  ...
                  {deal ? (
                    <button
                      type="button"
                      className="home-week-add"
                      onClick={() => addDealToCart(deal)}
                      disabled={
                        !dealProduct ||
                        Number(
                          dealProduct.stock ?? dealProduct.quantity ?? 1,
                        ) <= 0
                      }
                    >
                      <ShoppingCart size={11} />
                      Add · Rs.{" "}
                      {(isLive ? dealPrice : weeklyRegular).toLocaleString(
                        "en-PK",
                      )}
                    </button>
                  ) : null}
          ...
          <section className="home-big-deal">
            ...
            <div className="home-big-grid">
              <article className="home-big-card">
                <Link
                  className="home-big-image"
                  href={
                    bigDeal.productId
                      ? `/product/${bigDeal.productId}`
                      : "/deals/big"
                  }
                >
                  {src && (
                  ...
                  {stock > 0 && stock <= 10 ? (
                    <span className="home-urgency">Only {stock} left</span>
                  ) : null}
                  <span className="home-big-seal">
                    BIG
                    <br />
                    DEAL<small>OF THE DAY</small>
                  </span>
                </Link>
                <div className="home-big-info">
                  ...
                  <div className="home-big-prices">
                    ...
                    {regularPrice > price ? (
                      <s>Rs. {regularPrice.toLocaleString("en-PK")}</s>
                    ) : null}
                    {saved > 0 ? (
                      <em>Save Rs. {saved.toLocaleString("en-PK")}</em>
                    ) : null}
                  </div>
                  <small>Limited-time deal · Pakistan Time</small>
                  <button
                    className="home-add"
                    onClick={addBigDealToCart}
                    disabled={!live || !product || stock <= 0 || price <= 0}
                  >
    ...
    <>
      <section id="weekly-deals" className="mx-4 mt-3 scroll-mt-4">
        <div className="mb-3 flex items-center justify-between gap-3 rounded-[22px] border border-black/8 bg-white px-4 py-3 shadow-sm sm:px-5">
          <div>
            <p className="text-[8px] font-black uppercase tracking-[0.2em] text-[#E1352B]">
              Deals every day
            </p>
          ...
          <Link
            href="/weekly-deals"
            className="inline-flex shrink-0 items-center rounded-full bg-[#14140F] px-4 py-2.5 text-[9px] font-black uppercase tracking-[0.1em] text-white transition hover:bg-[#0F6A5F]"
          >
            View All Deals
          </Link>
        </div>
            ...
            {orderedDays.map(({ key, label, Icon }) => {
              ...
              return (
                <div
                  key={key}
                  className={
                    "group relative min-w-[145px] flex-1 overflow-hidden rounded-[20px] border-2 text-center transition duration-200 " +
                    cardClass
                  }
                >
                  {deal && dealImage ? (
                    ...
                    >
                      <span className="relative block aspect-[4/3] w-full overflow-hidden">
                        <Image
                          ...
                          sizes="(max-width: 640px) 145px, (max-width: 1024px) 20vw, 180px"
                          quality={72}
                          className="object-cover transition duration-200 group-hover:scale-105"
                        />
                        <span className="absolute left-1.5 top-1.5 rounded-full bg-[#E1352B] px-1.5 py-0.5 text-[6px] font-black uppercase tracking-[0.08em] text-white shadow-sm">
                          {isLive ? "Sale" : label}
                        </span>
                        {isLive && (
                          <span className="absolute bottom-1.5 left-1.5 rounded-full bg-emerald-500 px-1.5 py-0.5 text-[6px] font-black uppercase tracking-[0.08em] text-white shadow-sm">
                            LIVE
                          </span>
                        )}
                        {savings > 0 && (
                  ...
                  <span className="relative z-10 block px-2.5 pb-3 pt-2">
                    {deal?.productId ? (
                      ...
                      >
                        <span className="block whitespace-nowrap text-[10px] font-black uppercase tracking-[0.07em] text-[#14140F]">
                          {label.toUpperCase()}
                        </span>
                        {!isLive && (
                          <span className="mt-1 flex items-center justify-center gap-1 text-[7px] font-black uppercase tracking-[0.04em] text-black/55">
                            <LockKeyhole size={9} /> 🔒 Unlocks{" "}
                            {WEEKDAY_LABELS[key]}
                          </span>
                        )}
                        <span className="mt-1 block text-[7px] font-black uppercase tracking-[0.08em] text-[#E1352B]">
                          Deal Price
                        </span>
                        <span className="block text-[12px] font-black text-[#E1352B]">
                          Rs. {dealPrice.toLocaleString()}
                        </span>
                        <span className="mt-0.5 block text-[7px] font-black uppercase tracking-[0.08em] text-black/40">
                          Normal Price
                        </span>
                        <span className="block text-[9px] font-bold text-black/40 line-through">
                    ...
                    {deal && (
                      <button
                        ...
                        disabled={
                          !product ||
                          Number((product as ProductDealFields).stock ?? 1) <= 0
                        }
                    ...
                    {timing && !isLive && (
                      <span className="sr-only">
                        Unlocks {WEEKDAY_LABELS[key]} at the next weekly cycle.
                      </span>
                    )}
                  </span>
                </div>
      ...
      {bigDeal?.active &&
        ...
        (() => {
          const product = bigDeal.productId
            ? products[bigDeal.productId]
            : undefined;
          const productData = product as ProductDealFields | undefined;
          const deal = bigDeal;
          const title = deal.title;
          const currentPrice = Number(
            deal.dealPrice || productData?.dealPrice || productData?.price || 0,
          );
          const normalPrice = Number(
            deal.normalPrice ||
              productData?.originalPrice ||
          ...
          const savedAmount =
            normalPrice > currentPrice ? normalPrice - currentPrice : 0;
          const stock = Number(
            productData?.stock ?? productData?.quantity ?? deal.stock ?? 0,
          );
          const productImage = normalizeImageUrl(
            productData?.imageUrl || deal.imageUrl || "",
          );
          const productHref = deal.productId
            ...
            <section className="mx-4 mt-4 overflow-hidden rounded-[30px] border border-black/8 bg-white shadow-[0_20px_52px_rgba(20,20,15,0.12)]">
              ...
              >
                <div className="relative w-full aspect-square overflow-hidden rounded-[26px] bg-neutral-100 shadow-inner">
                  {productImage ? (
                    ...
                    <div className="flex h-full items-center justify-center text-sm font-black uppercase tracking-[0.18em] text-black/30">
                      Big Deal
                    </div>
                  )}
              ...
              <div className="bg-white px-5 py-5 sm:px-8 sm:py-6">
                <Link
                  ...
                  aria-label={`View ${title}`}
                >
                  <h2 className="line-clamp-2 text-2xl font-black leading-tight tracking-tight text-[#14140F] transition group-hover/title:text-[#0F6A5F] sm:text-4xl">
                    {title}
                  </h2>
                </Link>

                <div className="mt-4 flex flex-wrap items-center gap-2.5">
                  <span className="text-3xl font-black text-[#E1352B] sm:text-4xl">
                    Rs. {currentPrice.toLocaleString()}
                  </span>
                  {normalPrice > currentPrice && (
                    <span className="text-sm font-bold text-black/40 line-through sm:text-base">
                      Rs. {normalPrice.toLocaleString()}
                    </span>
                  )}
                </div>
