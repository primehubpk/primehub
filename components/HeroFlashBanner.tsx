      ...
      <div className="flex gap-1 overflow-x-auto px-3 py-3 sm:px-5 [scrollbar-width:none]">
        {DAYS.map(({ key, label, Icon }) => {
          ...
          return <Link key={key} href={`/deals/${key}`} className={`group relative min-w-[108px] flex-1 overflow-hidden rounded-2xl border px-2 py-2.5 text-center transition hover:-translate-y-0.5 ${active ? 'border-[#14140F] bg-[#14140F] text-white shadow-[0_8px_24px_rgba(20,20,15,0.14)]' : 'border-black/8 bg-white text-[#14140F] hover:border-[#FFB020] hover:shadow-[0_8px_24px_rgba(255,176,32,0.16)]'}`}>
            {deal?.imageUrl && <img src={deal.imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover opacity-25 transition group-hover:opacity-35" />}
            <span className="relative z-10 block">
              <span className={`mx-auto flex h-10 w-10 items-center justify-center overflow-hidden rounded-full border transition group-hover:scale-105 ${active ? 'border-white/20 bg-white/10 text-[#FFB020]' : 'border-[#FFB020]/40 bg-[#FFF8E8] text-[#D69200]'}`}>
              ...
              {deal && <span className={`mt-0.5 block text-[8px] font-bold ${active ? 'text-white/60' : 'text-black/40'}`}>Rs. {Number(deal.dealPrice).toLocaleString()}</span>}
            </span>
          </Link>;
        })}
      </div>
    ...
    <Link href="/deals/big" className="group mx-4 mt-3 block overflow-hidden rounded-[28px] bg-[#14140F] text-white shadow-[0_18px_50px_rgba(20,20,15,0.16)]">
      <section>
        <div className="relative min-h-[370px] overflow-hidden">
          {bigDeal?.imageUrl && <img src={bigDeal.imageUrl} alt={bigDeal.title || 'Big Deal'} className="absolute inset-0 h-full w-full object-cover opacity-75 transition duration-700 group-hover:scale-[1.02]" />}
          ...
          <div className="relative flex min-h-[370px] flex-col justify-end p-5 sm:p-7">
            ...
            <div className="max-w-xl">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-white/65">PrimeHub spotlight</p>
              <h2 className="text-3xl font-black leading-[0.95] tracking-tight sm:text-5xl">Big Deal. Limited Time.</h2>
              {bigDeal?.active && bigDeal.title ? <>
                <h3 className="mt-3 text-lg font-bold sm:text-2xl">{bigDeal.title}</h3>
                <div className="mt-4 flex items-end gap-3"><span className="font-[family-name:var(--font-mono)] text-3xl font-black text-[#FFB020]">Rs. {Number(bigDeal.dealPrice).toLocaleString()}</span>{Number(bigDeal.originalPrice) > Number(bigDeal.dealPrice) && <span className="pb-1 text-sm text-white/55 line-through">Rs. {Number(bigDeal.originalPrice).toLocaleString()}</span>}</div>
              </> : <p className="mt-3 max-w-md text-sm leading-6 text-white/65">Set a Big Deal from Admin → Settings to feature it here.</p>}
