...
import { CalendarDays, Clock3, Gift, Sparkles, Star, Tags, Trophy, WandSparkles, Zap } from 'lucide-react';
...
export default function HeroFlashBanner() {
  ...
  const countdown = useMemo(() => {
    ...
    const remaining = end > nowTick ? end - nowTick : millisecondsUntilPakistanMidnight();
    return countdownParts(remaining);
  ...
  const discount = dailyDeal && dailyDeal.originalPrice > dailyDeal.dealPrice
    ? Math.round(((dailyDeal.originalPrice - dailyDeal.dealPrice) / dailyDeal.originalPrice) * 100)
    : 0;
      ...
      <section className="mx-4 mt-3 overflow-hidden rounded-[24px] border border-black/5 bg-white shadow-[0_12px_40px_rgba(20,20,15,0.08)]">
        <div className="flex gap-1 overflow-x-auto px-3 py-3 sm:px-5 [scrollbar-width:none]">
          {DAYS.map(({ key, label, Icon }) => (
            <Link
              key={key}
              href={`/shop?dealDay=${key}`}
              className="group min-w-[108px] flex-1 rounded-2xl border border-black/8 bg-white px-2 py-2.5 text-center transition hover:-translate-y-0.5 hover:border-[#FFB020] hover:shadow-[0_8px_24px_rgba(255,176,32,0.16)]"
            >
              <span className="mx-auto flex h-10 w-10 items-center justify-center rounded-full border border-[#FFB020]/40 bg-[#FFF8E8] text-[#D69200] transition group-hover:scale-105">
                <Icon size={18} strokeWidth={2.2} />
              </span>
              <span className="mt-1.5 block whitespace-nowrap text-[10px] font-black uppercase tracking-[0.08em] text-[#14140F]">{label}</span>
            </Link>
          ))}
        </div>
      </section>
        ...
        <div className="relative min-h-[390px] overflow-hidden">
          ...
          <div className="relative flex min-h-[390px] flex-col justify-end p-5 sm:p-7">
            ...
            <div className="max-w-xl">
              <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.2em] text-white/65">Daily creative — managed from Admin</p>
              <h2 className="text-3xl font-black leading-[0.95] tracking-tight sm:text-5xl">Big Deal. Today Only.</h2>
              <h3 className="mt-3 text-lg font-bold sm:text-2xl">{dailyDeal?.title || 'No deal scheduled yet'}</h3>
              {dailyDeal && <div className="mt-4 flex items-end gap-3"><span className="font-[family-name:var(--font-mono)] text-3xl font-black text-[#FFB020]">Rs. {dailyDeal.dealPrice.toLocaleString()}</span>{dailyDeal.originalPrice > dailyDeal.dealPrice && <span className="pb-1 text-sm text-white/55 line-through">Rs. {dailyDeal.originalPrice.toLocaleString()}</span>}</div>}
              <div className="mt-4 flex flex-wrap items-center gap-2">
                <div className="inline-flex items-center gap-2 rounded-xl bg-white/10 px-3 py-2 backdrop-blur-md"><Clock3 size={14} /><span className="text-[10px] font-bold uppercase tracking-wider text-white/70">Ends in</span><span className="font-[family-name:var(--font-mono)] text-sm font-bold">{String(countdown.hours).padStart(2, '0')}:{String(countdown.minutes).padStart(2, '0')}:{String(countdown.seconds).padStart(2, '0')}</span></div>
                <Link href={dailyDeal?.buttonLink || '/shop'} className="inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-black text-[#14140F]">{dailyDeal?.buttonText || 'Shop products'} <Sparkles size={14} /></Link>
              </div>
            </div>
          </div>
        </div>
