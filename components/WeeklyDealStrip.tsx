...
export default function WeeklyDealStrip() {
  ...
  useEffect(() => {
    // STEP 03 source of truth: settings/main.weeklyDeals.
    // This replaces the STEP 15 duplicate `weeklyDeals` collection read.
          ...
          {orderedDeals.map((deal) => {
            ...
            return (
              <Link
                ...
                className={`min-w-[180px] overflow-hidden rounded-[24px] bg-white shadow-sm ${deal.day === today ? 'ring-2 ring-[#E1352B]' : ''}`}
              >
                <div className="relative h-28 bg-[#14140F]">
                  {deal.imageUrl && (
                    <img src={deal.imageUrl} alt={deal.title || `${dayLabel} Deal`} className="h-full w-full object-cover opacity-85" />
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent" />
                  <span className="absolute left-3 top-3 rounded-full bg-[#FFB020] px-2 py-1 text-[7px] font-black">{dayLabel}</span>
                  {deal.day === today && (
                    <span className="absolute right-3 top-3 rounded-full bg-[#E1352B] px-2 py-1 text-[7px] font-black text-white">TODAY</span>
                  )}
                </div>
                <div className="p-3">
                  <p className="truncate text-[10px] font-black">{deal.title || `${dayLabel} Deal`}</p>
                  ...
                  <p className="mt-2 flex items-center gap-1 text-[7px] font-bold text-black/40">
                    <Clock3 size={10} />
                    {deal.endAt && !Number.isNaN(new Date(deal.endAt).getTime()) ? `Ends ${new Date(deal.endAt).toLocaleString()}` : 'Limited time'}
                  </p>
