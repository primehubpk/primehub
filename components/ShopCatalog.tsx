      ...
      <div className="mx-auto max-w-6xl px-4 py-5 md:px-6">
            ...
            {buckets.map((bucket) => {
              ...
              return (
                <button
                  key={bucket.id}
                  type="button"
                  onClick={() => setMaxPrice(selected ? 'all' : String(bucket.amount))}
                  className={`relative min-w-[150px] shrink-0 overflow-hidden rounded-[22px] border p-3.5 text-left transition active:scale-[0.98] ${selected ? 'border-[#0F6A5F] bg-[#0F6A5F] text-white shadow-[0_14px_34px_rgba(15,106,95,0.18)]' : 'border-black/6 bg-white text-[#14140F] shadow-[0_10px_28px_rgba(20,20,15,0.06)]'}`}
                >
                  <div className="flex items-center justify-between">
                    <span className={`rounded-full px-2 py-1 text-[8px] font-black uppercase tracking-wider ${selected ? 'bg-white/12 text-white' : 'bg-[#FFF4F2] text-[#E1352B]'}`}>{bucket.title}</span>
                    <span className={`text-[9px] font-black ${selected ? 'text-white/70' : 'text-black/35'}`}>≤ Rs. {Number(bucket.amount).toLocaleString()}</span>
                  </div>
                  <div className="mt-4 flex items-end justify-between gap-3">
                    <div>
                      <p className={`text-[9px] font-bold ${selected ? 'text-white/60' : 'text-black/40'}`}>Best finds</p>
                      <p className="mt-0.5 text-lg font-black">Rs. {Number(bucket.amount).toLocaleString()}</p>
                    </div>
                    <ChevronRight className={`h-5 w-5 ${selected ? 'text-white/70' : 'text-black/20'}`} />
                  </div>
                </button>
              );
            })}
        ...
        <section className="mt-6">
          <div className="mb-3 flex items-end justify-between">
            ...
            <Link href="/category" className="flex items-center gap-0.5 rounded-full bg-white px-2.5 py-1.5 text-[10px] font-black text-[#0F6A5F] shadow-sm ring-1 ring-black/5">View all <ChevronRight size={14} /></Link>
          </div>
          <div className="flex gap-3 overflow-x-auto pb-1 [scrollbar-width:none]">
            {categories.map((cat) => {
              ...
              return (
                <button key={cat.id} type="button" onClick={() => setCategory(selected ? 'all' : value)} className="flex w-[78px] shrink-0 flex-col items-center gap-1.5 text-center">
        ...
        <div className="mt-5 flex items-center justify-between gap-2">
          ...
          <button type="button" onClick={() => setOnlyDeals((v) => !v)} className={`rounded-2xl px-4 py-3 text-[10px] font-black shadow-sm ring-1 ring-black/5 ${onlyDeals ? 'bg-[#E1352B] text-white' : 'bg-white text-[#14140F]'}`}>Flash deals only</button>
        </div>

        <section className="mt-4">
          {filtered.length === 0 ? (
            <div className="rounded-[28px] bg-white p-10 text-center shadow-sm ring-1 ring-black/5">
              <h2 className="text-base font-black">No products found</h2>
              <p className="mt-1 text-xs text-black/40">Try another search or clear the filters.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
              {filtered.map((product) => {
                ...
                return (
                  <Link key={product.id} href={`/product/${product.id}`} className="group overflow-hidden rounded-[22px] border border-black/6 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
                    <div className="relative aspect-square overflow-hidden bg-[#F4F4F1]">
                      {imageOf(product) ? <img src={imageOf(product)} alt={titleOf(product)} loading="lazy" className="h-full w-full object-cover transition duration-500 group-hover:scale-[1.03]" /> : <div className="flex h-full items-center justify-center text-[10px] font-bold text-black/25">No image</div>}
      ...
      {filtersOpen && (
        <div className="fixed inset-0 z-[100]">
          ...
          <div className="absolute bottom-0 left-0 right-0 rounded-t-[30px] bg-white p-5 shadow-2xl md:left-1/2 md:right-auto md:top-1/2 md:w-[420px] md:-translate-x-1/2 md:-translate-y-1/2 md:rounded-[30px]">
            <div className="flex items-center justify-between"><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-[#E1352B]">Refine</p><h2 className="mt-1 text-lg font-black">Shop Filters</h2></div><button type="button" onClick={() => setFiltersOpen(false)} className="flex h-9 w-9 items-center justify-center rounded-full bg-[#F4F4F1]"><X size={15} /></button></div>
            <div className="mt-6"><p className="text-[10px] font-black uppercase tracking-wider text-black/40">Price</p><div className="mt-2 grid grid-cols-4 gap-2">{[['all','Any'],['99','Under 99'],['499','Under 499'],['999','Under 999']].map(([value,label]) => <button key={value} type="button" onClick={() => setMaxPrice(value)} className={`rounded-2xl px-2 py-3 text-[9px] font-black ${maxPrice === value ? 'bg-[#14140F] text-white' : 'bg-[#F4F4F1] text-[#14140F]'}`}>{label}</button>)}</div></div>
            <button type="button" onClick={() => setOnlyDeals((v) => !v)} className={`mt-4 flex w-full items-center justify-between rounded-2xl p-4 text-left ${onlyDeals ? 'bg-[#E1352B] text-white' : 'bg-[#F4F4F1] text-[#14140F]'}`}><span><span className="block text-[11px] font-black">Flash Deals only</span><span className="mt-0.5 block text-[9px] opacity-60">Show products marked as flash sales</span></span><span className="text-[10px] font-black">{onlyDeals ? 'ON' : 'OFF'}</span></button>
            <button type="button" onClick={() => { setSearch(''); setCategory('all'); setMaxPrice('all'); setOnlyDeals(false); setFiltersOpen(false); }} className="mt-4 w-full rounded-full bg-[#14140F] py-3 text-[10px] font-black text-white">Clear all filters</button>
          </div>
        </div>
      )}
    </main>
