      ...
      <div className="relative overflow-hidden rounded-[24px] border border-black/[0.06] bg-gradient-to-br from-[#FFFDF8] via-[#F8F5EE] to-[#F1EEE7] shadow-[0_16px_42px_rgba(20,20,15,0.08)] sm:rounded-[34px]">
        ...
        <div className="relative p-4 sm:p-7 lg:p-8">
          <div className="min-w-0">
            <div className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-[#0F6A5F] text-white shadow-[0_10px_24px_rgba(15,106,95,0.24)] sm:h-14 sm:w-14"><Sparkles className="h-5 w-5 sm:h-6 sm:w-6"/></div>
            ...
            {ctaHref && <Link href={ctaHref} target="_blank" rel="noreferrer" className="mt-4 inline-flex min-h-10 items-center justify-center gap-2 rounded-full bg-[#181914] px-4 py-2.5 text-[10px] font-black text-white shadow-sm transition hover:-translate-y-0.5 sm:mt-5 sm:min-h-11 sm:px-5 sm:text-xs">{page.ctaText}<ArrowRight className="h-3.5 w-3.5 shrink-0"/></Link>}
          </div>
        </div>
      </div>
    </section>

    <section className="mx-auto mt-4 max-w-5xl sm:mt-7">{loading ? <div className="rounded-[20px] border border-black/5 bg-white px-4 py-10 text-center text-sm font-bold text-black/40">Loading skills...</div> : <div className="grid grid-cols-2 gap-2 sm:gap-5">{visibleItems.map((item, index) => {
      ...
      return <article key={item.id} className="group min-w-0 overflow-hidden rounded-[14px] border border-black/5 bg-white shadow-[0_7px_18px_rgba(20,20,15,0.06)] sm:rounded-[22px] sm:shadow-[0_12px_34px_rgba(20,20,15,0.07)]">
        <Link href={detailHref} className="block">
          <div className="relative aspect-video overflow-hidden bg-white">{thumbnail ? <Image src={thumbnail} alt={item.title || 'Prime Skill'} fill sizes="(max-width: 640px) 50vw, 480px" priority={index < 4} quality={72} className="object-contain transition duration-300 sm:object-cover sm:group-hover:scale-[1.02]"/> : <div className="flex h-full items-center justify-center text-black/25"><ImageIcon size={22}/></div>}</div>
          <div className="p-2 pb-0 sm:p-5 sm:pb-0"><h2 className="line-clamp-2 text-[11px] font-black leading-[15px] text-[#181914] sm:text-lg sm:leading-tight">{item.title}</h2>{priceInfo.price > 0 && <p className="mt-1 text-[10px] font-black text-[#E1352B] sm:mt-2 sm:text-sm">{priceInfo.from ? 'From ' : ''}Rs {priceInfo.price.toLocaleString()}</p>}{packageCount > 0 && <span className="mt-1.5 inline-flex rounded-full bg-[#0F6A5F]/10 px-2 py-1 text-[8px] font-black text-[#0F6A5F] sm:text-[9px]">{packageCount} package{packageCount === 1 ? '' : 's'} available</span>}</div>
        </Link>
