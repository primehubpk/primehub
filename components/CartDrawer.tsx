...
function productTitle(item: any) { return item.title || item.name || 'Product'; }
function productImage(item: any) { return item.imageUrl || item.image || item.images?.[0] || ''; }
function productPrice(item: any) { return Number(item.price || 0); }
...
export default function CartDrawer() {
  ...
  const subtotal = useMemo(() => items.reduce((sum: number, item: any) => sum + productPrice(item) * Number(item.quantity || item.qty || 1), 0), [items]);
  ...
  const sendWhatsApp = () => {
    ...
    const text = ['🛍️ PrimeHub Deals Order', '', ...lines, '', `Total Items: ${totalItems}`, `Subtotal: Rs. ${subtotal.toLocaleString()}`, '', 'I want to place this order via WhatsApp.'].join('\n');
          ...
          <div className="border-b border-black/8 bg-white px-5 py-4">
            <div className="mb-2 flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <span className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#0F6A5F]/10 text-[#0F6A5F]"><Truck size={15} /></span>
                <p className="text-[11px] font-black text-[#14140F]">{message}</p>
              </div>
              <span className="text-[10px] font-black text-black/40">{Math.min(totalItems, threshold)}/{threshold}</span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-black/8"><div className="h-full rounded-full bg-[#0F6A5F] transition-all duration-500" style={{ width: `${progress}%` }} /></div>
          </div>
        )}
          ...
          {items.length === 0 ? (
            ...
            <div className="space-y-3">
              {items.map((item: any) => {
                ...
                return (
                  <div key={id} className="rounded-[22px] border border-black/7 bg-white p-3 shadow-sm">
                    <div className="flex gap-3">
                      <div className="h-20 w-20 shrink-0 overflow-hidden rounded-2xl bg-[#F4F4F1]">{productImage(item) && <img src={productImage(item)} alt={productTitle(item)} className="h-full w-full object-cover" />}</div>
                      <div className="min-w-0 flex-1">
                        <p className="line-clamp-2 text-xs font-black text-[#14140F]">{productTitle(item)}</p>
                        <p className="mt-1 font-[family-name:var(--font-mono)] text-sm font-black text-[#E1352B]">Rs. {price.toLocaleString()}</p>
                        <div className="mt-2 flex items-center justify-between gap-2">
                          <div className="flex items-center rounded-xl bg-[#F4F4F1]">
                            <button type="button" onClick={() => qty <= 1 ? removeItem(id) : updateQty(id, qty - 1)} className="flex h-8 w-8 items-center justify-center" aria-label="Decrease quantity"><Minus size={13} /></button>
                            <span className="w-7 text-center text-[11px] font-black">{qty}</span>
                            <button type="button" onClick={() => updateQty(id, qty + 1)} className="flex h-8 w-8 items-center justify-center" aria-label="Increase quantity"><Plus size={13} /></button>
                          </div>
                          <button type="button" onClick={() => removeItem(id)} className="flex h-8 w-8 items-center justify-center rounded-xl text-black/35 hover:bg-[#E1352B]/10 hover:text-[#E1352B]" aria-label={`Remove ${productTitle(item)}`}><Trash2 size={14} /></button>
                        </div>
                      </div>
                    </div>
                  </div>
