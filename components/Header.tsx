...
function AnnouncementBar({ text }: { text: string }) {
  return <div className="bg-[#090909] py-2 text-white"><div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 text-[9px] font-black uppercase tracking-[0.13em] sm:px-6 sm:text-[10px]"><div className="flex min-w-0 items-center gap-4 overflow-x-auto whitespace-nowrap [scrollbar-width:none]"><span className="inline-flex items-center gap-1.5"><ShieldCheck size={12} className="text-[#FFB020]"/> Original Products</span><span className="hidden text-white/20 sm:inline">|</span><span className="inline-flex items-center gap-1.5"><Truck size={12} className="text-[#FFB020]"/> Fast Delivery</span><span className="hidden text-white/20 sm:inline">|</span><span className="inline-flex items-center gap-1.5"><Sparkles size={12} className="text-[#FFB020]"/> Premium Deals</span></div><span className="hidden shrink-0 text-[#FFB020] md:block">{text || 'PrimeHub Deals'}</span></div></div>;
}
