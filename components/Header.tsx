...
export default function Header() {
  ...
  return <>
    ...
    <div className="border-b border-black/8 bg-[#F8F7F3]"><div className="mx-auto max-w-7xl px-4 py-3.5 sm:px-6 lg:px-8"><div className="flex items-center gap-2.5 overflow-x-auto pb-1 [scrollbar-width:none]"><button type="button" onClick={() => router.push('/shop')} className="flex shrink-0 items-center gap-2 rounded-2xl border border-black/10 bg-[#14140F] px-3 py-2 text-[10px] font-black text-white shadow-sm"><MoreHorizontal size={15}/>All Categories</button><button type="button" onClick={() => router.push('/weekly-deals')} className="flex shrink-0 items-center gap-2 rounded-2xl border border-[#0F6A5F]/20 bg-[#E8F3F0] px-3 py-2 text-[10px] font-black text-[#0F6A5F] shadow-sm"><CalendarDays size={15}/>Weekly Deals</button>{visibleCategories.map((category) => <button key={category.id} type="button" onClick={() => openCategory(category)} className="group flex min-w-[88px] shrink-0 flex-col items-center gap-1.5 rounded-2xl px-2 py-1.5 text-[9px] font-black text-black/75"><span className="flex h-14 w-14 shrink-0 items-center justify-center overflow-hidden rounded-full bg-[#FFF0C9] text-[#B77900] ring-1 ring-black/5"><span className="block h-full w-full overflow-hidden rounded-full">{category.iconUrl ? <img src={category.iconUrl} alt="" className="h-full w-full object-cover" /> : <span className="flex h-full w-full items-center justify-center"><Sparkles size={18}/></span>}</span></span><span className="max-w-[96px] truncate">{category.title}</span></button>)}{categories.filter((category) => (category.active ?? true) && category.title.trim()).length > 12 && <button type="button" onClick={() => router.push('/shop')} className="flex shrink-0 flex-col items-center gap-1.5 rounded-2xl px-2 py-1.5 text-[9px] font-black"><MoreHorizontal className="h-14 w-14 rounded-full bg-[#FFF0C9] p-2.5 text-[#B77900]"/><span>More</span></button>}</div></div></div>
  </>;
}
