...
function AnnouncementBar({ text }: { text: string }) {
  ...
  const message = (
    <span className="inline-flex shrink-0 items-center gap-2">
      <ShieldCheck size={13} className="text-white/70" aria-hidden="true" />
      {announcement}
    </span>
  );

  return (
    <div className="overflow-hidden bg-[#090909] py-2.5 text-white" role="region" aria-label="Announcement">
      <div className="overflow-hidden whitespace-nowrap">
        <div className="flex w-max min-w-full animate-[ph-marquee_20s_linear_infinite]">
          <div className="flex min-w-full shrink-0 items-center justify-center gap-10 px-4 text-[10px] font-black uppercase tracking-[0.16em] sm:text-[11px]">
            {message}
          </div>
          <div className="flex min-w-full shrink-0 items-center justify-center gap-10 px-4 text-[10px] font-black uppercase tracking-[0.16em] sm:text-[11px]" aria-hidden="true">
            {message}
          </div>
        </div>
      </div>
      <style jsx>{`@keyframes ph-marquee{from{transform:translateX(0)}to{transform:translateX(-50%)}}`}</style>
    </div>
  );
}
...
export default function Header() {
  ...
  return <>
    ...
    <div className="sticky top-0 z-40 border-b border-black/8 bg-[#F8F7F3]/95 shadow-[0_6px_18px_rgba(20,20,15,0.05)] backdrop-blur-sm"><div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8"><div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none]"><button type="button" onClick={() => router.push('/shop')} className="flex shrink-0 items-center gap-2 rounded-2xl border border-black/10 bg-[#14140F] px-3 py-2 text-[10px] font-black text-white shadow-sm"><MoreHorizontal size={15}/>All Categories</button>{visibleCategories.map((category) => <button key={category.id} type="button" onClick={() => openCategory(category)} className="group flex min-w-[78px] shrink-0 flex-col items-center gap-1 rounded-2xl px-2 py-1.5 text-[9px] font-black text-black/75"><span className="flex h-8 w-8 items-center justify-center overflow-hidden rounded-full bg-[#FFF0C9] text-[#B77900]">{category.iconUrl ? <img src={category.iconUrl} alt="" className="h-full w-full object-cover"/> : <Sparkles size={14}/>}</span><span className="max-w-[84px] truncate">{category.title}</span></button>)}{categories.filter((category) => (category.active ?? true) && category.title.trim()).length > 12 && <button type="button" onClick={() => router.push('/shop')} className="flex shrink-0 flex-col items-center gap-1 rounded-2xl px-2 py-1.5 text-[9px] font-black"><MoreHorizontal className="h-8 w-8 rounded-full bg-[#FFF0C9] p-1.5 text-[#B77900]"/><span>More</span></button>}</div></div></div>
  </>;
}
