...
import { Heart, MoreHorizontal, Search, ShoppingCart, Tag, UserRound, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="bg-[#090909] py-2.5 text-white">
      <div className="mx-auto flex max-w-7xl items-center justify-center gap-2 px-4 text-center text-[10px] font-black uppercase tracking-[0.16em] sm:text-[11px]">
        <span className="shrink-0 text-white/70" aria-hidden="true">✦</span>
...
export default function Header() {
  ...
  return <>
    ...
    {/* Only the primary navigation is sticky. Category navigation intentionally scrolls away. */}
    <header className="sticky top-0 z-40 border-b border-black/10 bg-[#F8F7F3]/95 shadow-[0_8px_30px_rgba(20,20,15,0.06)] backdrop-blur-xl">
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
        ...
        <form onSubmit={submitSearch} className="mt-3 flex items-center rounded-full border border-black/10 bg-white px-4 py-2.5 shadow-[0_5px_18px_rgba(20,20,15,0.05)] md:hidden"><Search className="mr-2 h-4 w-4 shrink-0 text-black/35" aria-hidden="true" /><LiveSearchBar value={searchQuery} onChange={setSearchQuery} /><button type="submit" aria-label="Search" className="ml-2 flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#14140F] text-white"><Search size={14} /></button></form>
        <div className="mt-2 md:hidden"><div className="mb-1 flex items-center justify-between text-[10px] font-bold"><span className="text-[#0F6A5F]">{itemsToGo === 0 ? 'Free delivery unlocked!' : `Add ${itemsToGo} more item${itemsToGo === 1 ? '' : 's'} for FREE delivery`}</span><span className="text-black/40">{Math.min(cartItemCount, deliveryThreshold)}/{deliveryThreshold}</span></div><div className="h-1.5 overflow-hidden rounded-full bg-black/10"><div className="h-full rounded-full bg-[#0F6A5F] transition-all" style={{ width: `${deliveryProgress}%` }} /></div></div>
      </div>
    </header>
    ...
    <div className="border-b border-black/8 bg-[#F8F7F3]">
      <div className="mx-auto max-w-7xl px-4 py-3 sm:px-6 lg:px-8">
        <div className="flex items-center gap-2 overflow-x-auto pb-1 [scrollbar-width:none]">
          <button type="button" onClick={() => router.push('/shop')} className="flex shrink-0 items-center gap-2 rounded-2xl bg-[#14140F] px-3 py-2 text-[10px] font-black text-white shadow-sm"><MoreHorizontal size={15} />All Categories</button>
