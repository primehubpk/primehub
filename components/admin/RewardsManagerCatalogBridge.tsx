...
import PremiumRewardsManager from './PremiumRewardsManager';
...
export default function RewardsManagerCatalogBridge() {
  ...
  return (
    <>
      <PremiumRewardsManager products={filteredProducts} />
      <section className="mx-auto mb-6 max-w-6xl px-4 md:px-6">
        <div className="rounded-2xl border border-black/5 bg-white p-4 shadow-sm">
          <p className="text-[9px] font-black uppercase tracking-[.18em] text-[#E1352B]">Reward Store Product Picker</p>
          <h3 className="mt-1 text-sm font-black">Choose a category before linking products in the Reward Store</h3>
          ...
          <p className="mt-2 text-[10px] font-bold text-black/40">{category ? `${filteredProducts.length} product(s) available in this category.` : `${products.length} products loaded. Choose a category to filter the Reward Store product picker.`}</p>
        </div>
      </section>
    </>
  );
