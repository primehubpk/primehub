  ...
  return <>
    <div className="rounded-2xl border border-black/10 bg-[#FAFAF7] p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div><p className="text-sm font-black">Product image gallery</p><p className="text-[11px] text-black/45">Add images one by one. The first image is the main cover.</p></div>
        <label className={`inline-flex items-center gap-2 rounded-xl bg-[#14140F] px-4 py-2.5 text-xs font-black text-white ${busy ? 'pointer-events-none opacity-50' : 'cursor-pointer'}`}><ImagePlus size={14}/> {busy ? 'Processing…' : form.images.length ? 'Add another image' : 'Upload first image'}<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={onUpload} className="hidden" disabled={busy}/></label>
      </div>
      <div className="mt-4 grid grid-cols-3 gap-2 sm:grid-cols-5 lg:grid-cols-6">
        {(form.images.length ? form.images : [null]).map((url, index) => {
      ...
      <div className="mt-3 flex items-center gap-2 text-[10px] font-bold text-black/40"><Star size={12}/> Add as many images as you need. Images are compressed before upload.</div>
    </div>

    {showGallery && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4"><div className="max-h-[85vh] w-full max-w-5xl overflow-auto rounded-3xl bg-[#F4F4F1] p-5"><div className="flex items-center justify-between gap-3"><div><h3 className="text-xl font-black">Choose from Media Gallery</h3><p className="text-xs text-black/45">Select as many images as you need.</p></div><button type="button" onClick={onToggleGallery} className="rounded-full bg-white p-2"><X/></button></div><div className="mt-4 flex items-center gap-2 rounded-xl bg-white px-3 py-2"><Search size={15}/><input value={gallerySearch} onChange={e => onGallerySearchChange(e.target.value)} placeholder="Search gallery" className="w-full bg-transparent text-sm outline-none"/></div><div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-6">{gallery.map(asset => { const selected = form.images.includes(asset.url); return <button key={asset.id} type="button" onClick={() => selected ? onChangeImages(form.images.filter(url => url !== asset.url)) : onChangeImages([...form.images, asset.url])} className={`overflow-hidden rounded-xl border-2 bg-white text-left disabled:opacity-40 ${selected ? 'border-[#0F6A5F]' : 'border-transparent'}`}><img src={asset.url} alt={asset.name || 'Media'} className="aspect-square w-full object-cover"/><span className="block truncate p-2 text-[10px] font-bold">{selected ? '✓ Selected' : asset.name || 'Image'}</span></button>; })}</div><div className="mt-4 flex justify-end"><button type="button" onClick={onToggleGallery} className="rounded-xl bg-[#14140F] px-5 py-3 text-xs font-black text-white">Done</button></div></div></div>}
  </>;
}
