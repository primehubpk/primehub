...
import { CheckCircle2, Loader2, Plus, Trash2, ArrowUp, ArrowDown } from 'lucide-react';
...
export default function SiteSettingsManager() {
  ...
  const moveBucket = (index: number, direction: -1 | 1) => { const next = [...settings.priceBuckets]; const target = index + direction; if (target < 0 || target >= next.length) return; [next[index], next[target]] = [next[target], next[index]]; updateField('priceBuckets', next.map((bucket, i) => ({ ...bucket, sortOrder: i + 1 }))); };
  ...
  return <section className="mx-auto max-w-5xl px-4 py-6"><h2 className="text-2xl font-black">Store Settings</h2><p className="mt-1 text-sm text-black/55">Header, delivery, price buckets and customer-facing store settings are controlled here.</p><form onSubmit={saveSettings} className="mt-5 space-y-5 rounded-2xl border border-black/10 bg-white p-5">
    ...
    <div><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-bold">Glowing Price Buckets</p><p className="text-[11px] text-black/45">Admin controls title, max price, icon, glow accent, order and visibility. Homepage updates from these settings.</p></div><button type="button" onClick={addBucket} className="inline-flex items-center gap-1 rounded-full bg-[#14140F] px-3 py-2 text-[10px] font-black text-white"><Plus size={13}/>Add</button></div><div className="space-y-2">{settings.priceBuckets.map((bucket, index) => <div key={bucket.id} className="grid gap-2 rounded-xl bg-[#F4F4F1] p-3 sm:grid-cols-[1.2fr_100px_1.4fr_90px_72px_80px]"><input value={bucket.title} onChange={(e) => updateBucket(index, { title: e.target.value })} placeholder="Title" className="rounded-lg bg-white px-3 py-2 text-xs" /><input type="number" value={bucket.amount} onChange={(e) => updateBucket(index, { amount: Number(e.target.value) })} placeholder="Max price" className="rounded-lg bg-white px-3 py-2 text-xs" /><input value={bucket.iconUrl} onChange={(e) => updateBucket(index, { iconUrl: e.target.value })} placeholder="Icon image URL" className="rounded-lg bg-white px-3 py-2 text-xs" /><input type="text" value={bucket.accent} onChange={(e) => updateBucket(index, { accent: e.target.value })} placeholder="#FFB020" className="rounded-lg bg-white px-3 py-2 text-xs" /><label className="flex items-center gap-2 rounded-lg bg-white px-2 py-2 text-xs font-bold"><input type="checkbox" checked={bucket.active} onChange={(e) => updateBucket(index, { active: e.target.checked })}/>Show</label><div className="flex items-center gap-1"><button type="button" onClick={() => moveBucket(index, -1)} disabled={index === 0} aria-label="Move up" className="rounded-lg bg-white p-2 disabled:opacity-30"><ArrowUp size={13}/></button><button type="button" onClick={() => moveBucket(index, 1)} disabled={index === settings.priceBuckets.length - 1} aria-label="Move down" className="rounded-lg bg-white p-2 disabled:opacity-30"><ArrowDown size={13}/></button><button type="button" onClick={() => removeBucket(index)} aria-label={`Delete ${bucket.title}`} className="rounded-lg bg-white p-2 text-[#E1352B]"><Trash2 size={13}/></button></div></div>)}</div></div>
