...
import { Bot, Clock3, MessageCircle, PauseCircle, PlayCircle, Send, UserRound } from 'lucide-react';
...
type Thread = {
  conversation: Pick<Conversation, 'sessionId' | 'status' | 'holdType' | 'softHoldUntil' | 'needYou' | 'hasPending'>;
...
export default function SalaarInbox() {
  ...
  async function act(action: 'message' | 'wait' | 'continue', message?: string) {
        ...
        <aside className="max-h-[66vh] overflow-y-auto border-b border-black/8 bg-[#F8F7F3] md:border-b-0 md:border-r">
          {conversations.length ? conversations.map((item) => (
            <button
              ...
              className={`w-full border-b border-black/6 px-3 py-3 text-left transition ${selected === item.sessionId ? 'bg-white' : 'hover:bg-white/70'}`}
            >
            ...
            <>
              <div className="flex flex-wrap items-center gap-2 border-b border-black/8 px-3 py-3 sm:px-4">
                <div className="mr-auto min-w-0">
                  <p className="truncate text-sm font-black">{shortGuest(selected)}</p>
                  <p className="text-[9px] text-black/40">{thread.conversation.status}{thread.conversation.holdType ? ` · ${thread.conversation.holdType}` : ''}</p>
                </div>
                ...
                {thread.messages.map((message) => (
                  <div key={message.id} className={message.role === 'customer' ? 'mr-auto max-w-[86%]' : 'ml-auto max-w-[86%]'}>
                    <div className={`rounded-2xl px-3 py-2 text-xs leading-5 shadow-sm ${message.role === 'customer' ? 'rounded-bl-md bg-white text-black' : message.role === 'admin' ? 'rounded-br-md bg-[#14140F] text-white' : 'rounded-br-md bg-emerald-700 text-white'}`}>
                      <div className="mb-1 flex items-center gap-1 text-[8px] font-black uppercase opacity-60">
                        {message.role === 'customer' ? <UserRound size={9} /> : message.role === 'admin' ? <MessageCircle size={9} /> : <Bot size={9} />}
                        {message.role}{message.provider ? ` · ${message.provider}` : ''}{message.pending ? ' · pending' : ''}
                      </div>
                      {message.text}
                    </div>
