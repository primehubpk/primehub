...
export function useCategoriesManager() {
  ...
  useEffect(() => { getAdminDocument('settings', 'main').then((snapshot) => { const data = snapshot.exists() ? snapshot.data() : {}; const saved = Array.isArray(data.priceBuckets) ? data.priceBuckets as PriceBucket[] : []; setBuckets(saved.length ? saved.sort((a, b) => Number(a.sortOrder) - Number(b.sortOrder)) : DEFAULT_BUCKETS); }); }, []);
