...
export function useShopCatalog(initialCategory?: string, initialQuery = ''): ShopCatalogModel {
  const { settings } = useSettings(); const addItem = useCartStore((state) => state.addItem); const openVariantModal = useCartStore((state) => state.openVariantModal);
  ...
  const addProduct = (product: Product) => {
    const eventProduct = product as Product & { variants?: any[]; variantColors?: any; variantSizes?: any; hasVariants?: boolean };
    const hasVariants = Boolean(
      (eventProduct.variants && eventProduct.variants.length > 0) ||
      (eventProduct.variantColors && eventProduct.variantColors.length > 0) ||
      (eventProduct.variantSizes && eventProduct.variantSizes.length > 0) ||
      eventProduct.hasVariants,
    );
    if (hasVariants) {
      openVariantModal(eventProduct, 'cart');
      return;
    }
    const image = imageOf(product);
    addItem({ id: product.id, productId: product.id, name: titleOf(product), price: priceOf(product), originalPrice: originalOf(product) || priceOf(product), image, imageUrl: image });
