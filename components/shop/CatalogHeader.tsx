...
type Props = { search: string; count: number; setSearch: (value: string) => void; title?: string };
export default function CatalogHeader({ search, count, setSearch, title }: Props) {
  return <div className="rounded-[28px] bg-white p-5 shadow-[0_12px_40px_rgba(20,20,15,0.07)] ring-1 ring-black/5 md:p-7">
    ...
    <div className="mt-1 flex items-end justify-between gap-4"><div><h1 className="text-3xl font-black tracking-tight text-[#14140F] md:text-4xl">{title || 'Discover deals'}</h1><p className="mt-1 max-w-xl text-xs leading-5 text-black/45">Browse live products and prices from your PrimeHub store.</p></div><span className="rounded-full bg-[#F8F7F3] px-3 py-1.5 text-[9px] font-black text-black/50">{count} products</span></div>
