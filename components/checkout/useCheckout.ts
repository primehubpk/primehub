...
export function useCheckout(){
  ...
  const whatsappOrder=async()=>{if(!items.length)return;setError('');try{const[q,adminNumber]=await Promise.all([getAuthoritativeQuote(items),getAdminWhatsAppNumber()]);const user=auth.currentUser;let resellerCode='';let requestId='';if(user){const token=await user.getIdToken();const track=await fetch('/api/reseller/whatsapp-order',{method:'POST',headers:{'Content-Type':'application/json',Authorization:`Bearer ${token}`},body:JSON.stringify({customer,items:q.items})});const trackData=await track.json();if(track.ok){resellerCode=String(trackData.resellerCode||'');requestId=String(trackData.requestId||'');}else if(track.status!==401){throw new Error(trackData.error||'Unable to create WhatsApp reseller request.');}}
      const lines=q.items.map((validated:any,n:number)=>{const cartItem=items[n]||{},quantity=Number(cartItem.quantity||cartItem.qty||validated.quantity||1),cartPrice=Number(validated.price||priceOf(cartItem)),image=String(validated.image||validated.imageUrl||imageOf(cartItem)||'').trim();return[`${n+1}. ${titleOf(cartItem)||validated.title} x ${quantity}`,`- ${variantText(cartItem.variant||validated.variant)}`,`- Price: Rs. ${cartPrice.toLocaleString()}`,`- Image: ${image||'N/A'}`].join('\n');});
      const cartTotal=q.items.reduce((sum:number,item:any)=>sum+Number(item.price||0)*Number(item.quantity||1),0);
      const tracking=resellerCode?['','*RESELLER ORDER*',`Reseller Code: ${resellerCode}`,`Request ID: ${requestId}`]:[];
      const text=['*Order from PrimeHub*','',...lines,'','*Customer Details:*',`Name: ${customer.name||'-'}`,`Phone: ${customer.phone||'-'}`,`Address: ${customer.address||'-'}`,`City: ${customer.city||'-'}`,`*Total: Rs. ${cartTotal.toLocaleString()}*`,...tracking].join('\n');
