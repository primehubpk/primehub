...
async function getAdminWhatsAppNumber(){for(const ref of [doc(db,'settings','main'),doc(db,'settings','contact')]){try{const snap=await getDoc(ref);if(!snap.exists())continue;const data:any=snap.data()||{};const number=data.adminWhatsappNumber??data.whatsappNumber??data.whatsapp??data.whatsappPhone??data.contact?.adminWhatsappNumber??data.contact?.whatsappNumber??data.contact?.whatsapp??data.contact?.phone;const cleaned=cleanWhatsAppNumber(number);if(cleaned)return cleaned;}catch(error){console.warn('[whatsapp-checkout] Failed to read settings document',ref.id,error);}}throw new Error('Admin WhatsApp number is not configured.');}
