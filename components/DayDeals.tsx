...
function getDealImageUrl(deal: WeekendProduct) {
  ...
  return (typeof first === 'string' ? first : first?.url) || deal.imageUrl || deal.image || '/placeholder.png';
}
function isImgBB(src: string) {
  try {
    const url = new URL(src);
    return url.protocol === 'https:' && (url.hostname === 'i.ibb.co' || url.hostname === 'ibb.co');
  } catch {
    return false;
  }
}
...
export default function DayDeals() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(
      weekendDealsQuery,
      (snapshot) => setDeals(snapshot.docs.map((item) => ({ id: item.id, ...item.data() }) as WeekendProduct)),
      () => setDeals([]),
    );
  }, []);
        ...
        {deals.slice(0, 3).map((deal) => {
          ...
          return (
            <article key={deal.id} className="relative w-[180px] shrink-0 snap-start overflow-hidden rounded-xl border border-black/10 bg-white p-2 text-center sm:w-[210px]">
              ...
              <button type="button" onClick={(event) => addDeal(event, deal)} className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-xl bg-[#14140F] py-2.5 text-[9px] font-black text-white">
