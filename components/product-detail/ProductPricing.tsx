...
type Props = { product: Product; rating: number; reviews: number; currentDeal?: WeeklyDeal; liveDeal: boolean; dealPrice: number; regularPrice: number; productOriginal: number; normalForDeal: number; savingsAmount: number; stock: number; stockProgress: number };

export default function ProductPricing({ product, rating, reviews, currentDeal, liveDeal, dealPrice, regularPrice, productOriginal, normalForDeal, savingsAmount, stock, stockProgress }: Props) {
  ...
  return <>
    ...
    {currentDeal ? <div className="mt-5 overflow-hidden rounded-[26px] border border-[#E1352B]/15 bg-gradient-to-br from-white via-[#FCFCFA] to-[#FFF5F3] shadow-sm">
      <div className="bg-[#14140F] px-4 py-3 text-white sm:px-5">
        <div className="flex items-center justify-between gap-3">
          <span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.16em] text-white/65"><Zap size={12} fill="currentColor" />Weekly Flash Deal</span>
          <span className={`rounded-full px-2.5 py-1 text-[8px] font-black uppercase tracking-wider ${liveDeal ? 'bg-[#0F6A5F] text-white' : 'bg-white/10 text-white/70'}`}>{liveDeal ? 'Live now' : 'Locked'}</span>
        </div>
        <p className="mt-2 text-base font-black uppercase tracking-[0.08em]">{liveDeal ? `${dealDay} DEAL — LIVE NOW` : `UPCOMING ${dealDay} DEAL`}</p>
        {!liveDeal && <p className="mt-1 text-[9px] font-bold text-white/55">Special deal price unlocks on {dealDay}.</p>}
      </div>
      ...
      <div className="p-4 sm:p-5">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            <p className="text-[8px] font-black uppercase tracking-[0.18em] text-[#E1352B]">{liveDeal ? 'Deal price — live now' : 'Deal price when unlocked'}</p>
            <span className="font-[family-name:var(--font-mono)] text-3xl font-black tracking-tight text-[#E1352B] sm:text-4xl">{money(dealPrice)}</span>
          </div>
          <div className="text-right">
            <p className="text-[8px] font-black uppercase tracking-[0.16em] text-black/35">Current store price</p>
            <span className="text-sm font-black text-black/40 line-through">{money(normalForDeal)}</span>
          </div>
        </div>
        {savingsAmount > 0 && <div className="mt-3 flex flex-wrap items-center gap-2"><span className="rounded-full bg-[#0F6A5F] px-3 py-1.5 text-[9px] font-black text-white">SAVE {money(savingsAmount)}</span><span className="text-[10px] font-black text-[#0F6A5F]">You save {money(savingsAmount)}</span></div>}
        {!liveDeal ? <div className="mt-4 rounded-2xl bg-[#14140F]/[0.04] p-3"><p className="text-[9px] font-bold leading-4 text-black/55">Buy now at <span className="font-black text-[#14140F]">{money(regularPrice)}</span> or wait until the {dealDay} unlock for the special deal price.</p></div> : <div className="mt-4 flex items-center gap-2 rounded-2xl bg-[#0F6A5F]/10 px-3 py-2.5 text-[9px] font-black text-[#0F6A5F]"><Zap size={13} fill="currentColor" /> Special deal price is active now.</div>}
      </div>
