...
type Props = { product: Product; rating: number; reviews: number; currentDeal?: WeeklyDeal; liveDeal: boolean; dealPrice: number; regularPrice: number; productOriginal: number; normalForDeal: number; savingsAmount: number; afterPricing?: ReactNode };

export default function ProductPricing({ product, rating, reviews, currentDeal, liveDeal, dealPrice, regularPrice, productOriginal, normalForDeal, savingsAmount, afterPricing }: Props) {
  ...
  return <>
    ...
    {currentDeal ? <div className="mt-5 overflow-hidden rounded-[26px] border border-[#E1352B]/15 bg-gradient-to-br from-white via-[#FCFCFA] to-[#FFF5F3] shadow-sm">
      <div className="bg-[#14140F] px-4 py-3 text-white sm:px-5"><div className="flex items-center justify-between gap-3"><span className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-[0.16em] text-white/65"><Zap size={12} fill="currentColor" />Weekly Flash Deal</span><span className={`rounded-full px-2.5 py-1 text-[8px] font-black uppercase tracking-wider ${liveDeal ? 'bg-[#0F6A5F] text-white' : 'bg-white/10 text-white/70'}`}>{liveDeal ? 'Live now' : 'Locked'}</span></div><p className="mt-2 text-base font-black uppercase tracking-[0.08em]">{liveDeal ? `${dealDay} DEAL — LIVE NOW` : `UPCOMING ${dealDay} DEAL`}</p>{!liveDeal && <p className="mt-1 text-[9px] font-bold text-white/55">Special deal price unlocks on {dealDay}.</p>}</div>
      {!liveDeal && countdown && <div className="px-4 pt-4 sm:px-5"><p className="mb-2 text-center text-[8px] font-black uppercase tracking-[0.2em] text-black/40">Unlocks in</p><div className="grid grid-cols-4 gap-1.5 sm:gap-2">{[['days', countdown.days], ['hours', countdown.hours], ['minutes', countdown.minutes], ['seconds', countdown.seconds]].filter(([unit]) => unit !== 'days' || countdown.days > 0).map(([unit, value]) => <div key={String(unit)} className="rounded-2xl border border-black/6 bg-white px-1.5 py-2.5 text-center shadow-sm sm:px-2"><div className="font-[family-name:var(--font-mono)] text-base font-black leading-none text-[#14140F] sm:text-lg">{String(value).padStart(2, '0')}</div><div className="mt-1 text-[6px] font-black uppercase tracking-wider text-black/35 sm:text-[7px]">{unit === 'hours' ? 'hrs' : unit === 'minutes' ? 'min' : unit === 'seconds' ? 'sec' : 'days'}</div></div>)}</div></div>}
      <div className="p-4 sm:p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-[8px] font-black uppercase tracking-[0.18em] text-[#E1352B]">{liveDeal ? 'Deal price — live now' : 'Deal price when unlocked'}</p><span className="font-[family-name:var(--font-mono)] text-3xl font-black tracking-tight text-[#E1352B] sm:text-4xl">{money(dealPrice)}</span></div><div className="text-right"><p className="text-[8px] font-black uppercase tracking-[0.16em] text-black/35">Current store price</p><span className="text-sm font-black text-black/40 line-through">{money(normalForDeal)}</span></div></div>{savingsAmount > 0 && <div className="mt-3"><span className="inline-flex rounded-full bg-[#0F6A5F] px-3 py-1.5 text-[9px] font-black text-white">SAVE {money(savingsAmount)}</span></div>}{!liveDeal ? <div className="mt-4 rounded-2xl bg-[#14140F]/[0.04] p-3"><p className="text-[9px] font-bold leading-4 text-black/55">Buy now at <span className="font-black text-[#14140F]">{money(regularPrice)}</span> or wait until the {dealDay} unlock for the special deal price.</p></div> : <div className="mt-4 flex items-center gap-2 rounded-2xl bg-[#0F6A5F]/10 px-3 py-2.5 text-[9px] font-black text-[#0F6A5F]"><Zap size={13} fill="currentColor" /> Special deal price is active now.</div>}</div>
