...
export type ProductDetailState = {
  ...
  currentDeal?: WeeklyDeal;
  timing: ReturnType<typeof dealTiming> | null;
  liveDeal: boolean;
  dealPrice: number;
  normalForDeal: number;
...
export function imagesOf(product: Product) { const list = [...(Array.isArray(product.images) ? product.images : []), product.imageUrl, product.image].filter(Boolean) as string[]; return [...new Set(list)]; }
