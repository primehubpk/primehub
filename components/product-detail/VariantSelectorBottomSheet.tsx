...
export default function VariantSelectorBottomSheet({
  ...
  const selected = effectiveRows.find(
    (row) =>
    ...
    >
      <section className="w-full max-w-2xl animate-in slide-in-from-bottom duration-300 rounded-t-[28px] bg-white px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 shadow-2xl">
        ...
        <div className="max-h-[55vh] overflow-y-auto py-4 pr-1">
          <div>
            ...
            <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
              {colors.map((item) => {
                const active = item.name === color;
                const available = effectiveRows.some(
                  (row) => row.color === item.name && stockOf(row) > 0,
                );
                ...
                return (
                  <button
                    ...
                    disabled={!available}
                    className={`flex min-h-16 items-center gap-2 rounded-2xl border px-2.5 py-2 text-left transition-all duration-200 ${
                      active
                        ? 'border-[#0F6A5F] bg-[#0F6A5F]/[0.06] text-[#0F6A5F] ring-2 ring-[#0F6A5F] ring-offset-2'
                        : 'border-black/10 bg-white text-black/70'
                    } disabled:cursor-not-allowed disabled:opacity-30`}
                  >
                    <span className="h-10 w-10 shrink-0 overflow-hidden rounded-xl bg-[#F4F4F1]">
                      {item.imageUrl ? (
              ...
              {sizes.map((item) => {
                ...
                return (
                  <button
                    ...
                    disabled={!available}
                    className={`min-w-16 rounded-2xl border px-4 py-3 text-xs font-black transition-all duration-200 ${
                      active
        ...
        <button
          ...
          className={`flex min-h-14 w-full items-center justify-center gap-2 rounded-2xl py-4 text-xs font-black text-white shadow-lg transition-all disabled:cursor-not-allowed disabled:opacity-40 ${
            mode === 'buy' ? 'bg-[#E1352B]' : 'bg-[#14140F]'
          }`}
        >
