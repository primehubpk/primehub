...
import {
  MemberFeatureRail,
  ShopFeatureBanner,
} from "@/components/home/DiscoverFeatureRail";
...
const image = (p: Product) => {
  ...
  const raw =
    (typeof first === "object" ? first?.url : first) ||
    p.imageUrl ||
    p.image ||
    "";
...
const sameImage = (a: string, b: string) =>
  Boolean(
    a && b && normalizeImageUrl(a).trim() === normalizeImageUrl(b).trim(),
  );

function getModalProduct(p: Product): Product {
  const rawOptions = Array.isArray(p.options) ? p.options : [];
  const existingVariantOptions = Array.isArray(p.variantOptions)
    ? p.variantOptions
    : [];
  const normalizedOptions = rawOptions
    .map((option: any, index: number) => {
      ...
      const id = String(
        option.id ?? option.name ?? option.label ?? `option-${index}`,
      );
    ...
    .filter((option): option is { id: string; values: string[] } =>
      Boolean(option),
    );
...
export default function ProductGridRewards({
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, (user) => {
      ...
      if (user) {
        ...
        try {
          setPoints(
            Number(
              JSON.parse(localStorage.getItem(GUEST_KEY) || "{}")?.points || 0,
            ),
          );
        } catch {
          setPoints(0);
  ...
  function add(p: Product) {
    ...
    const hasVariants = Boolean(
      ...
      (Array.isArray(modalProduct.variantMatrix) &&
        modalProduct.variantMatrix.length > 0) ||
      (Array.isArray(modalProduct.variantColors) &&
        modalProduct.variantColors.length > 0) ||
      (Array.isArray(modalProduct.variantSizes) &&
        modalProduct.variantSizes.length > 0) ||
      (Array.isArray(modalProduct.variantOptions) &&
        modalProduct.variantOptions.length > 0) ||
      (Array.isArray(modalProduct.options) &&
        modalProduct.options.length > 0) ||
      ...
      <div className="mb-4 flex items-end justify-between gap-3">
        <div>
          ...
          <h2 className="text-2xl font-black tracking-tight">
            {homeLayout ? "PrimeHubMall Deals" : "Discover Deals"}
          </h2>
          ...
          {visible.map((p, index) => {
            ...
            return (
              <Fragment key={p.id}>
                    ...
                    <div className="absolute right-2.5 top-2.5 flex gap-1.5">
                      ...
                      >
                        <Heart
                          size={14}
                          className={
                            wished ? "text-[#E1352B]" : "text-[#14140F]"
                          }
                ...
                {!homeLayout && index === 5 && visible.length >= 6 ? (
                  <ShopFeatureBanner />
                ) : null}
                {!homeLayout && index === 11 && visible.length >= 12 ? (
                  <MemberFeatureRail />
                ) : null}
                {!homeLayout && index === 17 && visible.length >= 18 ? (
                  <PrimeSkillsHomeRail />
                ) : null}
              </Fragment>
            );
