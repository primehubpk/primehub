      ...
      <main className="flex min-h-screen items-center justify-center bg-[#F4F4F1] px-4">
        <form onSubmit={login} className="w-full max-w-sm rounded-[30px] bg-white p-7 shadow-xl">
          ...
          <label className="mt-5 block">
            <span className="mb-1.5 block text-[9px] font-black uppercase tracking-wider text-black/40">Admin Email</span>
            <input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full rounded-2xl bg-[#F4F4F1] px-3 py-3.5 text-xs font-bold outline-none" placeholder="admin@example.com" autoComplete="username" />
          </label>
          <label className="mt-3 block">
            <span className="mb-1.5 block text-[9px] font-black uppercase tracking-wider text-black/40">Password</span>
            <div className="flex items-center gap-2 rounded-2xl bg-[#F4F4F1] px-3">
              <LockKeyhole size={15} className="text-black/30" />
              <input required type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full bg-transparent py-3.5 text-xs font-bold outline-none" placeholder="Enter password" autoComplete="current-password" />
              <button type="button" onClick={() => setShowPassword((value) => !value)} className="p-1 text-black/40" aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff size={16} /> : <Eye size={16} />}</button>
            </div>
          </label>
    ...
    <div className="min-h-screen">
      ...
      {showSecurity && (
        <div className="border-b border-black/5 bg-white px-4 py-4">
          <form onSubmit={changePassword} className="mx-auto grid max-w-6xl gap-2 md:grid-cols-[1fr_1fr_auto] md:items-end">
            <label><span className="mb-1 block text-[9px] font-black uppercase tracking-wider text-black/40">Current password</span><input required type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className="w-full rounded-xl bg-[#F4F4F1] px-3 py-2.5 text-xs outline-none" autoComplete="current-password" /></label>
            <label><span className="mb-1 block text-[9px] font-black uppercase tracking-wider text-black/40">New password</span><div className="flex rounded-xl bg-[#F4F4F1] px-3"><input required minLength={8} type={showNewPassword ? 'text' : 'password'} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full bg-transparent py-2.5 text-xs outline-none" autoComplete="new-password" /><button type="button" onClick={() => setShowNewPassword((value) => !value)} aria-label="Toggle new password visibility" className="text-black/40">{showNewPassword ? <EyeOff size={15} /> : <Eye size={15} />}</button></div></label>
            <button disabled={savingPassword} className="rounded-xl bg-[#14140F] px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{savingPassword ? 'Saving...' : 'Change password'}</button>
          </form>
          {notice && <p className={`mx-auto mt-2 max-w-6xl text-[10px] font-bold ${notice.type === 'error' ? 'text-[#E1352B]' : 'text-[#0F6A5F]'}`}>{notice.text}</p>}
        </div>
      )}
      {children}
    </div>
