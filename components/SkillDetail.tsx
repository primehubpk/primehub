...
export default function SkillDetail({ skillId }: { skillId: string }) {
  ...
  return <main className="min-h-screen overflow-x-hidden bg-[#F4F4F1] px-3 pb-28 pt-3 sm:px-6 sm:pt-8">
    <div className="mx-auto max-w-5xl">
      ...
      <article className="overflow-hidden rounded-[20px] border border-black/5 bg-white shadow-[0_12px_34px_rgba(20,20,15,0.07)] sm:rounded-[28px]">
        ...
        <div className="p-4 sm:p-7">
          ...
          <div className="mt-5 flex flex-col gap-2 sm:flex-row sm:flex-wrap">
            {waHref && <Link href={waHref} target="_blank" rel="noreferrer" className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#0F6A5F] px-4 py-3 text-xs font-black text-white"><MessageCircle size={14}/>{item.buttonText || 'Order Place'}</Link>}
      ...
      {related.length > 0 && <section className="mt-6 sm:mt-8"><div className="mb-3"><p className="text-[8px] font-black uppercase tracking-[.18em] text-[#0F6A5F]">Prime Skills</p><h2 className="mt-1 text-base font-black sm:text-xl">Related Skills</h2></div><div className="grid grid-cols-2 gap-2.5 sm:gap-5">{related.map((entry) => <Link key={entry.id} href={`/skills/${encodeURIComponent(entry.id)}`} className="overflow-hidden rounded-[16px] border border-black/5 bg-white shadow-sm sm:rounded-[20px]"><div className="aspect-video bg-[#ECECE7]">{entry.thumbnailUrl ? <img src={entry.thumbnailUrl} alt={entry.title || 'Prime Skill'} className="h-full w-full object-cover" loading="lazy"/> : null}</div><div className="p-2.5 sm:p-4"><h3 className="line-clamp-2 text-[11px] font-black leading-[15px] sm:text-sm">{entry.title}</h3>{Number(entry.price || 0) > 0 && <p className="mt-1.5 text-[10px] font-black text-[#E1352B] sm:text-xs">Rs {Number(entry.price).toLocaleString()}</p>}</div></Link>)}</div></section>}
    </div>
  </main>;
}
