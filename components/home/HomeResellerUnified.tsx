...
export default function HomeResellerUnified() {
  ...
  async function startTask(task: ResellerTask) {
    ...
    if (task.id === "whatsapp-share") { const text = `${shareText}\n${shareUrl || window.location.origin}`.trim(); saveTaskEvent(task.id); setTaskEvents(readTaskEvents()); window.open(`https://wa.me/?text=${encodeURIComponent(text)}`, "_blank", "noopener,noreferrer"); return; }
    if (task.id === "refer-reseller") { const url = shareUrl || `${window.location.origin}/reseller/join?ref=${encodeURIComponent(user?.uid || guestId())}`; try { if (navigator.share) await navigator.share({ title: "PrimeHub Reseller Club", text: shareText, url }); else { await navigator.clipboard.writeText(`${shareText}\n${url}`); setMessage("Referral link copied — ab share karein."); } saveTaskEvent(task.id); setTaskEvents(readTaskEvents()); } catch {} return; }
    if (SOCIAL_TASK_IDS.has(task.id)) { if (user) { try { const token = await user.getIdToken(); await fetch("/api/reseller/task-claims", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` }, body: JSON.stringify({ taskId: task.id, action: "open" }) }); } catch {} } if (task.url) window.open(task.url, "_blank", "noopener,noreferrer"); else setMessage("Admin panel se is task ka platform link add karein."); return; }
