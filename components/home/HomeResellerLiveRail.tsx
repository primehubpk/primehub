...
import { CheckCircle2, ChevronRight, Gift, WalletCards } from "lucide-react";
...
type RewardGift = { id:string; title?:string; imageUrl?:string; pointsCost?:number; stock?:number; active?:boolean };
...
type Voucher = { id:string; title:string; requirement:string; imageUrl?:string };
...
export default function HomeResellerLiveRail(){
  ...
  useEffect(()=>onSnapshot(collection(db,"reward_gifts"),s=>setGifts(s.docs.map(d=>({id:d.id,...d.data()} as RewardGift)).filter(g=>g.active!==false&&Number(g.stock??1)>0)),()=>undefined),[]);
  ...
  const challenge={...DEFAULT_MONTHLY_CHALLENGE,...(source.resellerMonthlyChallenge||{})};
  const vi=source.resellerVoucherImages||{};
  const vouchers:Voucher[]=[
    {id:"cash-500",title:"Rs. 500 Cash",requirement:"5 orders",imageUrl:vi["cash-500"]},
    {id:"challenge-cash",title:`Rs. ${Number(challenge.cashReward||0).toLocaleString()} Cash`,requirement:`${challenge.targetOrders} orders`,imageUrl:vi["challenge-cash"]},
    {id:"challenge-gift",title:challenge.giftTitle||"PrimeHub Gift Box",requirement:`${challenge.targetOrders} orders`,imageUrl:vi["challenge-gift"]},
    {id:"bridal-gift",title:"Bridal Gift Voucher",requirement:"Gold tier",imageUrl:vi["bridal-gift"]},
    {id:"wholesale-off",title:"10% Wholesale Off",requirement:"3 orders",imageUrl:vi["wholesale-off"]},
    {id:"free-delivery",title:"Free Delivery",requirement:"2 orders",imageUrl:vi["free-delivery"]},
    {id:"jazzcash-300",title:"JazzCash Rs. 300",requirement:"8 orders",imageUrl:vi["jazzcash-300"]},
    {id:"easypaisa-300",title:"EasyPaisa Rs. 300",requirement:"8 orders",imageUrl:vi["easypaisa-300"]},
    {id:"kids-gift",title:"Kids Gift Box",requirement:"6 orders",imageUrl:vi["kids-gift"]},
    {id:"elite-cash",title:"Elite Cash",requirement:"Premium tier",imageUrl:vi["elite-cash"]},
  ];
  ...
  const count=Math.max(1,prizes.length); const imageSize=count<=2?58:count===3?50:count===4?44:count===5?40:count<=7?34:29; const radius=count<=3?31:count<=5?34:36;
  const topFixed=2+tasks.length+1; const bottomFixed=tiers.length+vouchers.length; const topGiftCount=Math.max(0,Math.min(gifts.length,Math.round((bottomFixed+gifts.length-topFixed)/2))); const topGifts=gifts.slice(0,topGiftCount); const bottomGifts=gifts.slice(topGiftCount);
  const GiftCard=({gift}:{gift:RewardGift})=><article className="ph-card ph-gift"><div className="ph-gift-art">{gift.imageUrl?<img src={gift.imageUrl} alt={gift.title||"Gift"}/>:<Gift size={28}/>}</div><small>REWARD GIFT</small><h4>{gift.title||"PrimeHub Gift"}</h4><p>{Number(gift.pointsCost||0).toLocaleString()} points</p></article>;

  return <section className="ph-live-rail" id="reseller-home"><HomeHeading>Reseller Club</HomeHeading><div className="ph-live-tabs"><a href="#reseller-rewards">Rewards</a><a href="#reseller-tasks">Tasks</a><a href="#reseller-tiers">Tiers</a><Link href="/reseller/dashboard">Open Club</Link></div><div className="ph-live-hint">Swipe → all tasks, tiers, vouchers & gifts</div><div className="ph-live-scroll"><div className="ph-live-grid">
    <div className="ph-live-row"><article className="ph-card ph-check" id="reseller-rewards"><small>WEEKLY STREAK</small><div className="ph-title"><h3>7-Day Check-in</h3><b>{streak}/7</b></div><div className="ph-days">{Array.from({length:7},(_,i)=><span key={i} className={i<streak?"done":""}>D{i+1}<b>+{rewardSettings.checkInRewards?.[i]||0}</b></span>)}</div><button onClick={()=>void checkIn()} disabled={busy||wallet.lastCheckIn===today}><CheckCircle2 size={14}/>{wallet.lastCheckIn===today?"Done today":"Check in"}</button></article>
    <article className="ph-card ph-wheel"><small>SPIN & WIN</small><h3>Your reward wheel</h3><div className="ph-wheel-wrap"><i></i><div className="ph-wheel-disc" style={{transform:`rotate(${rotation}deg)`}}>{prizes.map((p,i)=>{const angle=i*(360/count);return <div className="ph-prize" key={p.id} style={{transform:`translate(-50%,-50%) rotate(${angle}deg) translateY(-${radius}px)`,width:imageSize,height:imageSize}}>{p.imageUrl?<img src={p.imageUrl} alt={p.name}/>:<span>🎁</span>}</div>})}<em>WIN</em></div></div><button onClick={()=>void spin()} disabled={busy||usedSpin||!prizes.length}>{usedSpin?"Come tomorrow":busy?"Spinning…":"Spin the wheel"}</button></article>
    ...
    <article className="ph-card ph-wallet"><div className="ph-wallet-icon"><WalletCards size={18}/></div><small>YOUR WALLET</small><h4>{Number(wallet.points||0).toLocaleString()} points</h4><p>Rewards earned from check-ins, wheel and tasks.</p><Link href={user?"/reseller/dashboard":"/login?redirect=/reseller/dashboard"}>Open wallet <ChevronRight size={12}/></Link></article>{topGifts.map(g=><GiftCard key={g.id} gift={g}/>)}</div>
    <div className="ph-live-row">{tiers.map((tier,index)=>{const current=monthlyOrders>=tier.minMonthlyOrders&&(index===tiers.length-1||monthlyOrders<tiers[index+1].minMonthlyOrders);return <article className={`ph-card ph-tier ${current?"current":""}`} id={index===0?"reseller-tiers":undefined} key={tier.id}><span className="ph-tier-no">{index+1}</span><small>{current?"CURRENT":`${tier.minMonthlyOrders}+ monthly orders`}</small><h4>{tier.name}</h4><div><strong>{Number(tier.discountPercent||0)}%</strong><em> OFF</em></div><ul>{(tier.benefits||[]).slice(0,3).map(x=><li key={x}>✓ {x}</li>)}</ul></article>})}
    {vouchers.map(v=><article className="ph-card ph-voucher" key={v.id}>{v.imageUrl?<img src={v.imageUrl} alt={v.title}/>:<div className="ph-voucher-empty"><Gift size={25}/></div>}<small>VOUCHER</small><h4>{v.title}</h4><p>{v.requirement}</p></article>)}{bottomGifts.map(g=><GiftCard key={g.id} gift={g}/>)}</div>
  </div></div>{message?<div className="ph-live-message">{message}</div>:null}</section>;
}
