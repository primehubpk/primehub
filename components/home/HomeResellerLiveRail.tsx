...
type RewardPrize = { id:string; name:string; type:string; points?:number; probability?:number; active?:boolean; stock?:number; imageUrl?:string; productId?:string; voucherCode?:string; voucherAmount?:number };
...
export default function HomeResellerLiveRail({initialProducts=[]}:{initialProducts?:RewardProduct[]}){
  ...
  useEffect(()=>{if(!liveDataActive)return;let cancelled=false;fetch("/api/storefront/read?type=rewards",{cache:"no-store"}).then(response=>response.ok?response.json():null).then(data=>{if(cancelled||!data)return;const nextGifts=(Array.isArray(data.gifts)?data.gifts:[]).filter((gift:RewardGift)=>gift.active!==false&&Number(gift.stock??1)>0);setGifts(nextGifts);const next=(data.settings||{}) as RewardSettings;setLiveRewardSettings({guestMode:next.guestMode,checkInRewards:Array.isArray(next.checkInRewards)?next.checkInRewards:undefined,spinWheelSlots:Array.isArray(next.spinWheelSlots)?next.spinWheelSlots:[]});}).catch(()=>undefined);return()=>{cancelled=true};},[liveDataActive]);
