...
export default function HomeResellerLiveRail(){
  ...
  const count=Math.max(1,prizes.length); const sliceWidth=count<=2?78:count===3?66:count===4?56:count===5?49:count<=7?42:35;
  const topBase=2+tasks.length+1; const bottomBase=tiers.length+vouchers.length; const topGiftCount=Math.max(0,Math.min(gifts.length,Math.round((bottomBase+gifts.length-topBase)/2))); const topGifts=gifts.slice(0,topGiftCount); const bottomGifts=gifts.slice(topGiftCount);
