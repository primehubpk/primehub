...
import { ArrowRight, Crown, Gift, PackageSearch, ShieldCheck, ShoppingCart, Sparkles, Store, Tags, WalletCards } from 'lucide-react';
...
// Replace or extend this list when another home feature should take Cart's place.
const COMPANION_FEATURES: FeatureConfig[] = [{
  href: '/cart',
  eyebrow: 'Your Shopping',
  title: 'Cart',
  description: 'Review your selected products and continue securely to checkout.',
  benefits: [
    { icon: <ShoppingCart size={12}/>, label: 'Saved products' },
    { icon: <PackageSearch size={12}/>, label: 'Check quantities' },
  ],
  action: 'Open Cart',
  icon: <ShoppingCart size={23}/>,
  theme: 'bg-gradient-to-br from-[#FFF2D7] via-[#FFE4A8] to-[#FFCF68] text-[#14140F]',
}];

export function MemberFeatureRail() {
  return <section className="col-span-full my-3"><div className="mb-2 flex items-center justify-between px-1"><div><p className="text-[8px] font-black uppercase tracking-[.2em] text-[#0F6A5F]">More from PrimeHub</p><h3 className="mt-0.5 text-base font-black">Unlock more ways to shop</h3></div><span className="text-[8px] font-bold text-black/35 sm:hidden">Swipe →</span></div><div className="flex snap-x snap-mandatory gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden md:grid md:grid-cols-[1.35fr_.65fr] md:overflow-visible">{[RESELLER_FEATURE, ...COMPANION_FEATURES].map(feature => <FeatureCard key={feature.href} feature={feature}/>)}</div></section>;
}

function FeatureCard({ feature }: { feature: FeatureConfig }) {
  return <Link href={feature.href} className={`group relative min-h-[210px] w-[88%] shrink-0 snap-start overflow-hidden rounded-[26px] p-5 shadow-[0_14px_34px_rgba(20,20,15,0.10)] md:w-auto ${feature.theme}`}><div className="absolute -right-10 -top-10 h-32 w-32 rounded-full bg-white/10 blur-2xl"/><div className="relative flex h-full flex-col"><div className="flex items-start justify-between gap-3"><div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-white/15">{feature.icon}</div><span className="rounded-full bg-white/15 px-2.5 py-1 text-[7px] font-black uppercase tracking-wider">Featured</span></div><p className="mt-4 text-[8px] font-black uppercase tracking-[.2em] opacity-60">{feature.eyebrow}</p><h4 className="mt-1 text-xl font-black">{feature.title}</h4><p className="mt-1 max-w-xl text-[10px] leading-4 opacity-65">{feature.description}</p><div className="mt-3 flex flex-wrap gap-1.5">{feature.benefits.map(benefit => <span key={benefit.label} className="flex items-center gap-1 rounded-full bg-white/15 px-2.5 py-1.5 text-[8px] font-black">{benefit.icon}{benefit.label}</span>)}</div><div className="mt-auto flex items-center justify-between pt-4 text-[10px] font-black"><span>{feature.action}</span><span className="flex h-9 w-9 items-center justify-center rounded-full bg-white text-[#14140F] transition group-hover:translate-x-1"><ArrowRight size={15}/></span></div></div></Link>;
}
