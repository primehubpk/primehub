...
type ChatMessage = { id: string; role: 'user'|'assistant'|'system'; text: string; attachments?: { products?: ProductAttachment[]; images?: ImageAttachment[]; links?: LinkAttachment[] } | any[]; created_at?: string };
...
export default function SalarWidget() {
  ...
  async function sendPayload(payload: Record<string, unknown>) {
    ...
    try {
      ...
      setMessages((current) => [...current, ...(Array.isArray(data?.messages) ? data.messages : [])]);
      setInput('');
    ...
    {open ? <section className={`salar-panel ${maximized ? 'salar-panel-max' : ''}`} aria-label="Salar help chat">
      ...
      {adminDrawer && adminLoggedIn ? <div className="salar-admin-drawer"><SalarChats compact /></div> : <>
        <div className="salar-body"><div className="salar-messages">{!messages.length && !busy ? <div className="salar-bubble">Assalamualaikum, I am Salar from PrimeHub Mall. Need help?</div> : null}
          {messages.map((message) => { const products = productsOf(message); const images = imagesOf(message); const links = linksOf(message); return <div key={message.id} className={`salar-message-wrap ${message.role === 'user' ? 'salar-message-mine' : ''}`}>
